...
if($_GET["Command"]=="save_item")
{
	...
	while ($_GET["mcou"]>$i){
		...
		if (($_GET[$itemcode]!="") and ($_GET[$itemname]!="") and ($_GET[$retqty]>0)){
			...
			$sql= "UPDATE s_rawmas SET acc_cost =".$avgcost.", QTYINHAND = QTYINHAND - ".$_GET[$retqty]."  WHERE STK_NO='".$_GET[$itemcode]."'";
			$result=mysql_query($sql, $dbinv);
			...
			if ($result==false){ $sql_status=4; }	 
   			...
   			$sql= "iNSERT INTO s_rawtrn (STK_NO, SDATE, QTY, LEDINDI, REFNO, DEPARTMENT) VALUES('".$_GET[$itemcode]."', '".$_GET["grndate"]."', ".$_GET[$retqty].", 'ARR', '".$_GET["pur_ret_no"]."', '".$_GET["department"]."')";
			$result=mysql_query($sql, $dbinv);
			//echo $sql;
			if ($result==false){ $sql_status=1; }	 
...
if($_GET["Command"]=="cancel_inv")
{
	...
	while($row = mysql_fetch_array($result)){
		...
		$sql= "UPDATE s_rawmas SET QTYINHAND = QTYINHAND + ".$row["REC_QTY"]."  WHERE STK_NO='".$row["STK_NO"]."'";
		$result =$db->RunQuery($sql);
		echo $sql;		 
		...
		$result =$db->RunQuery($sql);
  		...
  		$sql= "delete from s_rawtrn where STK_NO='".$row["STK_NO"]."' and LEDINDI='ARR' and DEPARTMENT='".$_GET["department"]."' and REFNO='".$_GET["pur_ret_no"]."'";
   		echo $sql;
