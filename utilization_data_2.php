...
if ($_GET["Command"]=="settle_inv"){
	...
	$ResponseXML .= "<uti_table><![CDATA[ <table  border=1  cellspacing=0>
        ...
        $sqlOuter = "select Inv_no from s_crnfrmtrn where crd_note = '" .$_GET["txtcrnno"]. "'"; 
        $resultOuter =$db->RunQuery($sqlOuter);
        
        $i=1;
        while ($rowOuter = mysql_fetch_array($resultOuter)){
                
                $sql="select * from s_salma where REF_NO='".$rowOuter["Inv_no"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0  ORDER BY SDATE";
        ...
        $sqlOuter = "select INVOICENO from s_crnma where REF_NO = '" .$_GET["txtcrnno"]. "'"; 
        $resultOuter =$db->RunQuery($sqlOuter);
        
        while ($rowOuter = mysql_fetch_array($resultOuter)){
                
                $sql="select * from s_salma where REF_NO='".$rowOuter["INVOICENO"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0  ORDER BY SDATE";
	...
	if ($i==1){
            ...
            $sqlOuter = "select C_INVNO from cred where C_REFNO = '" .$_GET["txtcrnno"]. "'"; 
            $resultOuter =$db->RunQuery($sqlOuter);

            while ($rowOuter = mysql_fetch_array($resultOuter)){

                    $sql="select * from s_salma where REF_NO='".$rowOuter["C_INVNO"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0  ORDER BY SDATE";
...
if($_POST["Command"]=="save_crec")
{
	...
	if ($result_utmas!=1){ $sql_status=1; }
        ...
        $l_lmem = "Utilization " . trim($_POST["txt_cusname"]);
        $ldate = $_POST["dtdate"];
        require_once './gl_posting.php';
        $ayear = ac_year($ldate);
        $fulltot = $_POST["lblPaid"] + $_POST["txtcash"];

        $sql19 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear) values ('" . trim($recno) . "', '" . $ldate . "', 'UT_CRE_ACC', " . $fulltot . ", 'UT', 'CRE', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear')";
        //echo $sql;
        ...
        $sql20 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear) values ('" . trim($recno) . "', '" . $ldate . "', 'UT_DEB_ACC', " . $fulltot . ", 'UT', 'DEB', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear')";
        //echo $sql;
...
if ($_GET["Command"]=="delete_rec"){
	...
	$sql="DELETE from ledger WHERE l_refno='" . trim($_GET["txtrefno"]) . "'";
	$result=mysql_query($sql, $dbinv);	
