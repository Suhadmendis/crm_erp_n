	...
	if (date("Y", strtotime($_GET["DTPicker1"])) > 2012) {
		...
		if($row_rsper= mysql_fetch_array($result_rsper)){
        	...
        	while ($ii < $TypeGrid1_count){
            	...
            	if (($_GET[$TypeGrid14] == "INV") or ($_GET[$TypeGrid14] == "INV - TBR") or ($_GET[$TypeGrid14] == "INV - TBB")) {
                	if ($_GET[$TypeGrid09] <= $_GET[$TypeGrid10]) {
						if ($_GET["txttotal"]>=500000){
                    		$TypeGrid[$ii][13] = ($val_TypeGrid08 / 100 * 3) / ($_GET["txt_vat"] + 100) * 100;
						} else if (($_GET["txttotal"]<500000) and ($_GET["txttotal"]>=350000)){
							$TypeGrid[$ii][13] = ($val_TypeGrid08 / 100 * 2.5) / ($_GET["txt_vat"] + 100) * 100;
						} else {
							$TypeGrid[$ii][13]=0;
						}	
					...
					if ($_GET["txttotal"]>=500000){
                		$TypeGrid[$ii][13] = -1*($val_TypeGrid03 / 100 * 3) / ($_GET["txt_vat"] + 100) * 100;
					} else if (($_GET["txttotal"]<500000) and ($_GET["txttotal"]>=350000)){
						$TypeGrid[$ii][13] = -1*($val_TypeGrid03 / 100 * 2.5) / ($_GET["txt_vat"] + 100) * 100;
					} else {
						$TypeGrid[$ii][13] = 0;
					}	
            	}
	...
	if (date("Y", strtotime($_POST["DTPicker1"])) > 2012) {
		...
		if($row_rsper= mysql_fetch_array($result_rsper)){
        	...
        	while ($ii < $TypeGrid1_count){
            	...
            	if (($_POST[$TypeGrid14] == "INV") or ($_POST[$TypeGrid14] == "INV - TBR") or ($_POST[$TypeGrid14] == "INV - TBB")) {
                	if ($_POST[$TypeGrid09] <= $_POST[$TypeGrid10]) {
						if ($_POST["txttotal"]>=500000){
                    		$TypeGrid13 = ($_POST[$TypeGrid08] / 100 * 3) / ($_POST["txt_vat"] + 100) * 100;
						} else if (($_POST["txttotal"]<500000) and ($_POST["txttotal"]>=350000)){
							$TypeGrid13 = ($_POST[$TypeGrid08] / 100 * 2.5) / ($_POST["txt_vat"] + 100) * 100;
						} else {
							$TypeGrid13 = 0;
						}		
                	} else {
					...
					if ($_POST["txttotal"]>=500000){
                		$TypeGrid13 = -1*($_POST[$TypeGrid03] / 100 * 3) / ($_POST["txt_vat"] + 100) * 100;
					} else {
						$TypeGrid13 = -1*($_POST[$TypeGrid03] / 100 * 2.5) / ($_POST["txt_vat"] + 100) * 100;
					} else {
						$TypeGrid13 = 0;
					}	
            	}
		...
		if ($row_rsper= mysql_fetch_array($result_rsper)){
        	...
        	while ($ii < $TypeGrid1_count){
            	...
            	if (($_POST[$TypeGrid14] == "INV") or ($_POST[$TypeGrid14] == "INV - TBR") or ($_POST[$TypeGrid14] == "INV - TBB")) {
                	if ($_POST[$TypeGrid09] <= $_POST[$TypeGrid10]) {
						if ($_POST["txttotal"]>=500000){
                    		$TypeGrid[$ii][13] = ($_POST[$TypeGrid08] / 100 * 3) / ($_POST["txt_vat"] + 100) * 100;
						} else if (($_POST["txttotal"]<500000) and ($_POST["txttotal"]>=350000)){
							$TypeGrid[$ii][13] = ($_POST[$TypeGrid08] / 100 * 2.5) / ($_POST["txt_vat"] + 100) * 100;
						}	else {
							$TypeGrid[$ii][13] = 0;
						}
                	} else {
					...
					if ($_POST["txttotal"]>=500000){
                		$TypeGrid[$ii][13] = -1*($_POST[$TypeGrid03] / 100 * 3) / ($_POST["txt_vat"] + 100) * 100;
					} else if (($_POST["txttotal"]<500000) and ($_POST["txttotal"]>=350000)){	
						$TypeGrid[$ii][13] = -1*($_POST[$TypeGrid03] / 100 * 2.5) / ($_POST["txt_vat"] + 100) * 100;
					} else {
						$TypeGrid[$ii][13] = 0
					}	
            	}
...
if($_GET["Command"]=="proces")
{
	...
	//$count_rssslma= mysql_num_rows($sql_rssslma);
	$xx = $count_rssslma;
	...
	$sql_RScbal = "select * from c_bal where ((month(SDATE)='" . intval(date("m", strtotime($_GET["DTPicker1"]))) . "' and  year(SDATE)='" . date("Y", strtotime($_GET["DTPicker1"])) . "') or (month(SDATE)='" . intval(date("m", strtotime($_GET["DTPicker2"]))) . "' and  year(SDATE)='" . date("Y", strtotime($_GET["DTPicker2"])) . "') or (month(SDATE)='" . intval(date("m", strtotime($_GET["DTPicker3"]))) . "' and  year(SDATE)='" . date("Y", strtotime($_GET["DTPicker3"])) . "')) and CANCELL='0'and CUSCODE='" . trim($_GET["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' order by SDATE";
	...
	while ($tmp_count<$ii){
		...
		$TypeGrid1[$tmp_count][12]=0;
		$TypeGrid1[$tmp_count][13]=0;
		
		if ($TypeGrid1[$tmp_count][9] <= $TypeGrid1[$tmp_count][10]) {
		
			if ($txttotal>=500000){
        		$TypeGrid1[$tmp_count][12] = -1*($TypeGrid1[$tmp_count][3] / 100 * 3) / ($txt_vat + 100) * 100;
				$TypeGrid1[$tmp_count][13] = -1*($TypeGrid1[$tmp_count][3] / 100 * 3) / ($txt_vat + 100) * 100;
			} else if (($txttotal<500000) and ($txttotal>=350000)){
				$TypeGrid1[$tmp_count][12] = -1*($TypeGrid1[$tmp_count][3] / 100 * 2.5) / ($txt_vat + 100) * 100;
				$TypeGrid1[$tmp_count][13] = -1*($TypeGrid1[$tmp_count][3] / 100 * 2.5) / ($txt_vat + 100) * 100;
			} else {
				$TypeGrid1[$tmp_count][12] = 0;
				$TypeGrid1[$tmp_count][13] = 0;
			}
		...
		if ($array_incen[$ii]=="salma"){
			$totincen = $totincen + $TypeGrid1[$ii][12];	
		} else if ($array_incen[$ii]=="cbal"){	
			$tgrnince = $tgrnince + $TypeGrid1[$ii][12];
        	$txttot_inc = $txttot_inc + $TypeGrid1[$ii][12];	
		}	
	...
	if (date("Y", strtotime($_GET["DTPicker1"])) > 2012) {
    	$sql_rsper= "Select * from intper where incen_year = 2013 and traget < " . (($txttot1 - $txt_grn) / ($txt_vat + 100) * 100) . " order by traget desc ";
		...
		if($row_rsper= mysql_fetch_array($result_rsper)){
        	...
        	while ($ii < $TypeGrid1_count){
            	if (($TypeGrid1[$ii][14] == "INV") or ($TypeGrid1[$ii][14] == "INV - TBR") or ($TypeGrid1[$ii][14] == "INV - TBB")) {
                	if ($TypeGrid1[$ii][9] <= $TypeGrid1[$ii][10]) {
						if ($txttot1>=500000){
                    		$TypeGrid1[$ii][13] = ($TypeGrid1[$ii][8] / 100 * 3) / ($txt_vat + 100) * 100;
						} else if (($txttot1<500000) and ($txttot1>=350000)){
							$TypeGrid1[$ii][13] = ($TypeGrid1[$ii][8] / 100 * 2.5) / ($txt_vat + 100) * 100;
						} else {
							$TypeGrid1[$ii][13] = 0;
						}	
                	} else {
					...
					if ($txttot1>=500000){
                		$TypeGrid1[$ii][13] = -1*($TypeGrid1[$ii][3] / 100 * 3) / ($txt_vat + 100) * 100;
					} else if (($txttot1<500000) and ($txttot1>=350000)){
						$TypeGrid1[$ii][13] = -1*($TypeGrid1[$ii][3] / 100 * 2.5) / ($txt_vat + 100) * 100;
					} else {
						$TypeGrid1[$ii][13] = 0;
					}	
            	}
...
if($_GET["Command"]=="calc1_table")
{
	...
	if (($_GET["TypeGrid14"] == "INV") or ($_GET["TypeGrid14"] == "INV - TBR") or ($_GET["TypeGrid14"] == "INV - TBB")) {
   		...
   		if ($_GET["TypeGrid09"] <= $_GET["TypeGrid10"]) {
			if ($TypeGrid08>=500000){
      			$TypeGrid13 = ($TypeGrid08 / 100 * 3) / ($_GET["txt_vat"] + 100) * 100;
			} else if (($TypeGrid08<500000) and ($TypeGrid08>=350000)){
				$TypeGrid13 = ($TypeGrid08 / 100 * 2.5) / ($_GET["txt_vat"] + 100) * 100;
			} else {
				$TypeGrid13 = 0;
			}		
		...
		if ($TypeGrid08>=500000){
   			$TypeGrid13 = -($TypeGrid03 / 100 * 3) / ($_GET["txt_vat"] + 100) * 100;
		} else if (($TypeGrid08<500000) and ($TypeGrid08>=350000)){
			$TypeGrid13 = -($TypeGrid03 / 100 * 2.5) / ($_GET["txt_vat"] + 100) * 100;
		} else {
			$TypeGrid13 =0;
		}	
   		$txttot_inc=$txttot_inc+$TypeGrid13;
