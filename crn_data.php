...
if($_GET["Command"]=="GL"){
			...
			$sql="Select C_CODE,C_NAME from lcodes where C_CODE = '". $_GET["accno2"] ."'";
			$result =$db->RunQuery($sql);
			$row = mysql_fetch_array($result);
			 
			$ResponseXML .= "<acc1><![CDATA[".$row["C_CODE"]."]]></acc1>";
			$ResponseXML .= "<accn1><![CDATA[".$row["C_NAME"]."]]></accn1>";



			$sql="Select DebCntAcc from vendor where CODE = '". $_GET["cus_code"] ."'";
			$result =$db->RunQuery($sql);
			$row = mysql_fetch_array($result);

			$sql="Select C_CODE,C_NAME from lcodes where C_CODE = '". $row["DebCntAcc"] ."'";
			$result =$db->RunQuery($sql);
			$row = mysql_fetch_array($result);

			$ResponseXML .= "<acc2><![CDATA[".$row["C_CODE"]."]]></acc2>";
			$ResponseXML .= "<accn2><![CDATA[".$row["C_NAME"]."]]></accn2>";
			
			$sql="Select C_CODE,C_NAME from lcodes where C_CODE = '22201001-000'";
			$result =$db->RunQuery($sql);
			$row = mysql_fetch_array($result);


			$ResponseXML .= "<acc3><![CDATA[".$row["C_CODE"]."]]></acc3>";
			$ResponseXML .= "<accn3><![CDATA[".$row["C_NAME"]."]]></accn3>";
			
			$sql="Select C_CODE,C_NAME from lcodes where C_CODE = '22201002-000'";
			$result =$db->RunQuery($sql);
			$row = mysql_fetch_array($result);

			
			$ResponseXML .= "<acc4><![CDATA[".$row["C_CODE"]."]]></acc4>";
			$ResponseXML .= "<accn4><![CDATA[".$row["C_NAME"]."]]></accn4>";
...
if($_GET["Command"]=="delete_crn")
{
  ...
  echo $ResponseXML;
   ...
   //               $sql1="insert into s_sttr(ST_REFNO, ST_DATE, ST_INVONO, ST_PAID, balance, netamount, cus_code, cusname, sal_ex, DEV, del_days, deliin_days, deliin_amo, deliin_lock, department, form_type, trn_type) values
	  // ('".$_GET["crnno"]."', '".$_GET["crndate"]."', '".$_GET["crnno"]."', ".$_GET["amount"].", ".$_GET["amount"].", ".(-1*$_GET["amount"]).", '".$_GET["cus_code"]."', '".$_GET["cus_name"]."', '".$_GET["salesrep"]."',  '".$_SESSION['dev']."', 0, 0, 0, '0', '".$_GET["department"]."', 'CNT', 'OVER')";
			// $result1=mysql_query($sql1, $dbinv);
                ...
                // if ($tableAry[$i]['Pay'] != null) {

                 if ($_GET["DE"] == "0") {
	            
	            	if ($tableAry[$i]['GPAY'] > 0) {
	            		# code...
		                $sql1 = "insert into s_sttr(ST_REFNO, ST_DATE, ST_INVONO, ST_PAID) values
		                        ('" . $_GET["crnno"] . "','" . $tableAry[$i]['Date'] . "','" . $_GET["crnno"] . "','" . $tableAry[$i]['GPAY'] . "')";
		                $result1=mysql_query($sql1, $dbinv);
