...
<table id="items">';
 ...
 $item = "";
    foreach ($conn->query($sql3) as $row4) {
        ...
        if ($Location != $row4['Location']) {


//        $countsql = "select count(Item_Name) from quotation_table where Item_Name = '". $row4['Item_Name'] ."'";
...
//    
            ...
            echo '<td colspan="4"><b>' . $row4['Location'] . '</b></td>';
            $Location = $row4['Location'];
            $item = $row4['Item_Name'];
            echo '<tr class="item-row">
        <td >' . $row4['Item_Name'] . '</td>
        <td style="text-align: center;"> ' . $row4['Qty'] . '</td>
        <td style="text-align: center;"> ' . $row4['Unit_Price'] . '</td>
        <td style="text-align: center;"> ' . $row4['Unit_Price']*$row4['Qty'] . ' +NBT+VAT</td>
