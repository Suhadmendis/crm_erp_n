...
function getno() {
    ...
    return $invno = trim("QTN/") . substr($tmpinvno, $lenth - 7);
}
...
//            $rowItem = $conn->query($sql)->fetch();
            ...
            $sql5 = "Insert into s_trn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, cost) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'QTN','" . $_GET["from_dep"] . "', '" . $row["str_description"] . "', " . $row["val"] . ")";
            $conn->exec($sql5);
        }
        /*
        ...
        */
        ...
        $sql1 = "insert into s_ginmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE, C_CODE, CUS_NAME, C_ADD1) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '', '', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'QTN', '" . $_GET["cusText"] . "', '" . $_GET["cusTextName"] . "', '".$row["ADD1"]."')";
        $conn->exec($sql1);
