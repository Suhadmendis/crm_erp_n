    ...
    <body>
        ...
        function htmlview() {
            ...
            if ($mcount == 1) {
                ...
                $limit = 0;
                $result_crLmt1 = $db->RunQuery($crLmt);
                while ($row_crLmt1 = mysql_fetch_array($result_crLmt1)) {
            ...
            echo "<center><b>" . $stringhesd . "</b></center><br>";
/*
...
//                    echo "<tr><td>" . trim($row["REF_NO"]) . "</td>";
                    $trn = "INV";
                    echo "<tr><td><a href=\"\" class=\"INV\" onClick=\"NewWindow('sales_inv_display.php?refno=" . $row['REF_NO'] . "&trn_type=" . $trn . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\"> " . $row['REF_NO'] . "</a></td>";
                    echo "<td>" . trim($row["dele_no"]) . "</td>";
                    ...
                    $d120 = date('Y-m-d', strtotime(($row["SDATE"]) . ' +'.$lmt.' days'));
                    echo "<td align=right >" . $d120 . "</td>";
            ...
            if ($_GET["cmbbrand1"] != "All") {
                ...
                if ($_GET["cmbrep"] != "All") {
                    $sqlTest = "SELECT rep FROM vendor where CODE = '" . $_GET['cuscode'] . "'";
                    $resultTest = $db->RunQuery($sqlTest);
                    $rowTest = mysql_fetch_array($resultTest);
                    if ($rowTest[0] == $_GET['cmbrep']) {
                        $sql_rst4 = "SELECT * FROM s_invcheq WHERE che_date>'" . date("Y-m-d") . "' AND CUS_CODE='" . trim($_GET["cuscode"]) . "' and trn_type='REC' and sal_ex='01' ";
                    }
                }
            ...
            if ($_GET["cmbbrand1"] == "All") {
                ...
                if ($_GET["cmbrep"] != "All") {
                    ...
                    $rowTest = mysql_fetch_array($resultTest);
                    if ($rowTest[0] == $_GET['cmbrep']) {
                        $strsql = "Select * from s_invcheq where cus_code='" . $_GET["cuscode"] . "'  and che_date>'" . date("Y-m-d") . "' and sal_ex='01' order by che_date";
                    }
                }
            ...
            if($_SESSION["company"]!="A"){
                echo "************************************</br></br></br>";
                echo "***Cash Discount Conditions***</br></br></br>";
                echo "- Payment more than 120 days - No Discount</br></br>";
                echo "- Payment made less than 120 days - 10% Discount</br></br>";
                echo "- No Discount for all Japanese Products</br></br>";
                echo "(TOTO Piston &amp; Liner, TP Rings, Rocky Valve, TOYO U. Joint)</br></br>";
            }
            echo "</body>";
            echo "</html>";
        }
        ...
        function printut() {
            ...
            if (($_GET["cmbbrand1"] == "All") and ( $_GET["cmbGRNtype"] == "All") and ( trim($_GET["cuscode"]) == "") and ( $_GET["cmbrep"] == "All")) {
                $sql = "select * from view_c_bal_s_deftrn where  BALANCE>0 AND CANCELL = '0' and trn_type<>'ARN'  and sdate>'2011-01-01' order by sdate ";
            }
            if (($_GET["cmbbrand1"] == "All") and ( $_GET["cmbGRNtype"] == "All") and ( trim($_GET["cuscode"]) != "") and ( $_GET["cmbrep"] == "All")) {
...
//echo $sql;
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                echo "<td>" . $row["CUSCODE"] . "-" . $rowv['NAME'] . "</td>";
