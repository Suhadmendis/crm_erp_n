...
if ($_GET["Command"]=="pass_cus_cash_rec")
{
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			if ($_GET['refno']==""){
				$sql = mysql_query("Select C_CODE, REF_NO, deli_date, GRAND_TOT, TOTPAY, SDATE from s_salma where C_CODE='".$_GET['custno']."' and GRAND_TOT > TOTPAY and (GRAND_TOT-TOTPAY)>0.5 and CANCELL='0' and DEV='".$_SESSION["dev"]."' ORDER BY SDATE,REF_NO") or die(mysql_error());
			} else {
			
				$sql = mysql_query("Select C_CODE, REF_NO, deli_date, GRAND_TOT, TOTPAY, SDATE from s_salma where C_CODE='".$_GET['custno']."' and GRAND_TOT > TOTPAY and (GRAND_TOT-TOTPAY)>0.5 and CANCELL='0' and SAL_EX='".$_GET['refno']."' and DEV='".$_SESSION["dev"]."'  ORDER BY SDATE,REF_NO") or die(mysql_error());
			}	
        ...
        if ($invset > 0) {
            if ($invset <= $chqbal) {
				...
				$invset_val=str_replace(",", "", $invset);	
   				...
   				$sql1="insert into tmp_utilization(recno, invno, invdate, org_invdate, chqno, chqdate, chbank, settled, days, invval, c1, tmp_no) values ('".$_POST["recno"]."', '".$_POST[$invno]."', '".$date1."', '".$_POST[$sdate]."', '".$a_chq_no[$i]."', '".$a_chq_date[$i]."', '".$a_chq_bank[$i]."', ".$invset_val.", ".$days.", ".$col2.", 0, '".$_SESSION["tmp_no_cashrec"]."')"; 
			//echo $sql1." 1 /";
				...
				$sql1="insert into tmp_utilization(recno, invno, invdate, org_invdate, chqno, chqdate, chbank, settled, days, invval, c1, tmp_no) values ('".$_POST["recno"]."', '".$_POST[$invno]."', '".$date1."', '".$_POST[$sdate]."', '".$a_chq_no[$i]."', '".$a_chq_date[$i]."', '".$a_chq_bank[$i]."', ".$chqbal_val.", ".$days.", ".$col2.", 0, '".$_SESSION["tmp_no_cashrec"]."')"; 
			//echo $sql1." 2 /";
				$result =$db->RunQuery($sql1);
...
while ($_POST["mcount"]>$ii){
  ...
  if ($_POST[$cash_pay] != "") {
	...
	$sql1="insert into tmp_utilization(recno, invno, invdate, org_invdate, chqno, chqdate, chbank, settled, days, invval, c1, tmp_no) values ('".$_POST["recno"]."', '".$_POST[$invno]."', '".$date1."', '".$_POST[$sdate]."', '".$_POST["paytype"]."', '".$_POST["invdate"]."', '".$a_chq_bank[$i]."', ".$cash_pay_val.", ".$days.", ".$col2.", 0, '".$_SESSION["tmp_no_cashrec"]."')"; 
			//echo $sql1." 3 /";
	$result =$db->RunQuery($sql1);
     $K = $K + 1;
  }
...
if($_POST["Command"]=="save_crec")
{
	...
	if ($_SESSION["tmp_no_cashrec"]!=""){
		...
		mysql_query("SET AUTOCOMMIT=0", $dbacc);	 
...
if ($recno !=''){
	...
	while ($row = mysql_fetch_array($result)){
	  ...
	  ('".$recno."', '".$_POST["invdate"]."', '".$row["invno"]."', ".$settled.", '".$row["chqno"]."', '".$ST_CHDATE."', '".$ST_flag."', '".$row["days"]."', '".$row["days"]."', '".$row["chbank"]."', '".$_POST["cuscode"]."', '".$_SESSION['dev']."', 0, 0, 0, '0', 'O')";
	//  echo $sql1;
	  ...
	  ('".$recno."', '".$_POST["invdate"]."', '".$row["invno"]."', ".$settled.", ".(-1*$settled).", '".$row["chqno"]."', '".$ST_CHDATE."', '".$ST_flag."', '".$row["days"]."', '".$row["days"]."', '".$row["chbank"]."', '".$_POST["cuscode"]."', '".$_POST["cusname"]."', '".$_POST["salesrep"]."', '".$_SESSION['dev']."', 0, 0, 0, '0', 'O', 'REC', 'SET')";
	//  echo $sql1;
	  	$result1=mysql_query($sql1, $dbinv);
	...
	if ($_POST["txtoverpay"]>0){
	 ...
	 // echo $sql;
	  ...
	  ('".$recno."', '".$_POST["invdate"]."', '".$recno."', ".$_POST["txtoverpay"].", ".$_POST["txtoverpay"].", ".(-1*$_POST["txtoverpay"]).", '".$_POST["cuscode"]."', '".$_POST["cusname"]."', '".$_POST["salesrep"]."', '".$_SESSION['dev']."', 0, 0, 0, '0', 'REC', 'OVER')";
		//echo $sql1;
		$result1=mysql_query($sql1, $dbinv);
		if ($result1!=1){ $sql_status=12; }	
		
	}
	if ($_SESSION['dev']=="0"){
		$sql2="update invpara set INV_RECNO=INV_RECNO + 1";
		$result2=mysql_query($sql2, $dbinv);
		if ($result2!=1){ $sql_status=13; }	
	}
	
	if ($_SESSION['dev']=="1"){
		$sql2="update invpara set INV_RECNO_M=INV_RECNO_M + 1";
		$result2=mysql_query($sql2, $dbinv);
		if ($result2!=1){ $sql_status=14; }	
	} 	
  ...
  //if (($_POST["paytype"]=="J/Entry") or ($_POST["paytype"]=="Cash TT") ){	
  if  ($_POST["paytype"]=="Cash TT") {	
	...
	if (trim($_POST["ledg_ref_no"])!=""){
		...
		if ($result_rst!=1){ $sql_status=15; }	
	}	
	
	$sql_rst="delete from bankdepmas where refno='" . trim($recno) . "'";
	$result_rst=mysql_query($sql_rst, $dbacc);
	if ($result_rst!=1){ $sql_status=16; }	
	...
	//echo $sql;
	$result=mysql_query($sql, $dbacc);
	if ($result!=1){ $sql_status=20; }	
	
	
	$sql="delete from bankdeptrn where refno='" . trim($recno) . "'";
	 $result=mysql_query($sql, $dbacc);
	 if ($result!=1){ $sql_status=21; }	
	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mfield"]=="recno"){
								...
								$sql = mysql_query("SELECT * from s_crec where   DEV='".$_SESSION["dev"]."' and CA_REFNO like  '$letters%' limit 50") or die(mysql_error());
							} else if ($_GET["mstatus"]=="recdate"){
								$letters = $_GET['recdate'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and CA_DATE like  '$letters%' limit 50") or die(mysql_error());
							}else if ($_GET["mstatus"]=="recamt"){
								$letters = $_GET['recamt'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and CA_AMOUNT like  '$letters%' limit 50") or die(mysql_error());
							} else {
								$letters = $_GET['recno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and CA_REFNO like  '$letters%' limit 50") or die(mysql_error());
...
if ($_GET["Command"]=="search_rec_simple"){
	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mfield"]=="recno"){
								...
								//echo $a;
								//echo "SELECT * from s_crec where  CA_REFNO and DEV='".$_SESSION["dev"]."' like  '$letters%' limit 50";
								$sql = mysql_query("SELECT * from s_crec where   DEV='".$_SESSION["dev"]."' and company = '".$_SESSION["company"]."' and CA_REFNO like  '$letters%' limit 50") or die(mysql_error());
							} else if ($_GET["mstatus"]=="recdate"){
								$letters = $_GET['recdate'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and company = '".$_SESSION["company"]."' and CA_DATE like  '$letters%' limit 50") or die(mysql_error());
							}else if ($_GET["mstatus"]=="recamt"){
								$letters = $_GET['recamt'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and company = '".$_SESSION["company"]."' and CA_AMOUNT like  '$letters%' limit 50") or die(mysql_error());
							} else {
								$letters = $_GET['recno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_crec where DEV='".$_SESSION["dev"]."' and company = '".$_SESSION["company"]."' and CA_REFNO like  '$letters%' limit 50") or die(mysql_error());
							...
							while($row = mysql_fetch_array($sql)){
								...
								$stname = $_GET["mstatus"];
							$ResponseXML .= "<tr>
                           	  <td onclick=\"recnoSimple('$REF_NO');\">".$row['CA_REFNO']."</a></td>
                              <td onclick=\"recnoSimple('$REF_NO');\">".$row['CA_DATE']."</a></td>
                              <td onclick=\"recnoSimple('$REF_NO');\">".$row['CA_AMOUNT']."</a></td>";
							  
							  $sql1="SELECT * FROM vendor where CODE = '".$row["CA_CODE"]."'";
							  $result1 =$db->RunQuery($sql1);
							  $row1 = mysql_fetch_array($result1);
                              $ResponseXML .= "<td onclick=\"recnoSimple('$REF_NO');\">".$row1['NAME']."</a></td>                          	
                            </tr>";
							}
...
if ($_GET["Command"]=="pass_recno_simple"){
	header('Content-Type: text/xml'); 
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
	...
	$sql_tmpinst= " delete from tmp_cash_credit_note_form where crn_form_no='".$_GET["txtrefno"]."' and username='$userName'";
	//echo $sql_tmpinst;
   ...
   // Do While MSFlexGrid1.TextMatrix(i, 1) <> ""
        ...
        $sqlPre="select distinct ST_INVONO from s_sttr where ST_REFNO = '$recno'";
        $resultLoop = $db->RunQuery($sqlPre);
//        echo $sqlPre;
        while ($rowLoop = mysql_fetch_array($resultLoop)){
        
        $sql_rscrn= " Select * from s_crnfrmtrn where Inv_no = '" .$rowLoop["ST_INVONO"]. "' and Flag = 'CCRN' order by Inv_date";
//		echo $sql_rscrn;
		...
		if ($row_rscrn = mysql_fetch_array($result_rscrn)){
			...
			$sql_rssalma = "Select * from view_s_sttr_copy where ST_INVONO = '" . trim($rowLoop["ST_INVONO"]) . "' AND ST_REFNO = '$recno' AND (ST_FLAG = 'CHK' or ST_FLAG ='Cash TT' or ST_FLAG ='CAS' or ST_FLAG ='UT') order by SDATE";
//			echo $sql_rssalma;
			$result_rssalma =$db->RunQuery($sql_rssalma);
			
        	if ($row_rssalma = mysql_fetch_array($result_rssalma)){
                        $cusCode = $row_rssalma["C_CODE"];
                        $cusName = $row_rssalma["CUS_NAME"];
        	   	$result_rssalma =$db->RunQuery($sql_rssalma);
//                        echo $sql_rssalma;
        		while ($row_rssalma = mysql_fetch_array($result_rssalma)){
                	if ((is_null($row_rssalma["deli_date"])==false) && ($row_rssalma["deli_date"]!="0000-00-00")){
					...
					} if (($row_rssalma["ST_FLAG"]=="CHK") or ($row_rssalma["ST_FLAG"]=="Cash TT")or ($row_rssalma["ST_FLAG"]=="J/Entry")or ($row_rssalma["ST_FLAG"]=="CAS")){	
...
//				echo $sql_tmpinst;
        	...
        	while ($row = mysql_fetch_array($result)){
				...
				<td ><input type=\"text\" size=\"10\" name=".$Incen_per."  onkeyup='caldis($i);' id=".$Incen_per." value='".$row['Incen_per']."' class=\"text_purchase3_right\"/></td>
				...
				<td ><input type=\"text\" size=\"10\" name=".$disc1." onBlur=\"cal_incentive_val_cash('".$i."')\" onkeypress=\"cal_incentive_val_cash('".$i."')\"  id=".$disc1." value='".$row['disc1']."' class=\"text_purchase3_right\"/></td>
			...
			$ResponseXML .= "<recno><![CDATA[$recno]]></recno>";
			$ResponseXML .= "<cuscode><![CDATA[$cusCode]]></cuscode>";
			$ResponseXML .= "<cusname><![CDATA[$cusName]]></cusname>";
	...
	if (date("m")==date("m", strtotime($_GET["invdate"]))){
		...
		if ($row = mysql_fetch_array($result)){
			if ($row["AMOUNT"]==$row["BALANCE"]){
                ...
                //supporting dapp sync ----
                $sql = "select ST_INVONO from s_sttr where ST_REFNO='" . $_GET["recno"] . "'";
                $result = mysql_query($sql, $dbinv);
                while ($row = mysql_fetch_array($result)) {
                    $insert[] = "('" . $row["ST_INVONO"] . "', '" . $_GET["recno"] . "')";
                }

                $sql = "insert into s_sttr_inv_cancel (invno,REF_NO) values " . implode(',', $insert);
