...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
    ...
    //PRICE_EDIT=true      ' edit selling price
		...
		if($_GET["Command"]=="new_inv")
		{
			...
			$sql="Select CCRNNO from invpara where COMCODE = '" .$_SESSION['COMCODE']. "'";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
						...
						if ($_GET["mstatus"]=="invno"){ 
						  ...
						  if ($_SESSION["slected"]=="all"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Refno like  '$letters%' and cancell = '0' and flag = 'CCRN' and company = '" . $_SESSION["company"]. "' order BY Refno desc limit 120") or die(mysql_error());
							
						  } else if ($_SESSION["slected"]=="locked"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Refno like  '$letters%' cancell = '0' and Lock1='1' and flag = 'CCRN' and company = '" . $_SESSION["company"]. "' order BY Refno desc limit 120") or die(mysql_error());
							
		
						  } else if ($_SESSION["slected"]=="pending"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Refno like  '$letters%' cancell = '0' and Lock1='0' and flag = 'CCRN' and company = '" . $_SESSION["company"]. "' order BY Refno desc limit 120") or die(mysql_error());
							
		
						  }	
	 					}
						
						if ($_GET["mstatus"]=="customername"){ 
							$letters = $_GET['customername']; 
						  if ($_SESSION["slected"]=="all"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Code like  '$letters%' and cancell = '0' and flag = 'CCRN' order BY Refno desc limit 120") or die(mysql_error());
							
						  } else if ($_SESSION["slected"]=="locked"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Code like  '$letters%' cancell = '0' and Lock1='1' and flag = 'CCRN' order BY Refno desc limit 120") or die(mysql_error());
							
		
						  } else if ($_SESSION["slected"]=="pending"){
						  	$sql = mysql_query("select Refno, Code, Amount   from s_crnfrm where Code like  '$letters%' cancell = '0' and Lock1='0' and flag = 'CCRN' order BY Refno desc limit 120") or die(mysql_error());
...
if ($_GET["Command"]=="pass_crn_form"){
   ...
   // Do While MSFlexGrid1.TextMatrix(i, 1) <> ""
		...
		if ($row_rscrn = mysql_fetch_array($result_rscrn)){
			...
			} if (($row_rsstr["ST_FLAG"]=="CHK") or ($row_rsstr["ST_FLAG"]=="Cash TT")or ($row_rsstr["ST_FLAG"]=="J/Entry")or ($row_rsstr["ST_FLAG"]=="CAS")){	
...
//        echo $sql_rscrn;
		...
		if ($row_rscrn = mysql_fetch_array($result_rscrn)){
			...
			//	if ($row1 = mysql_fetch_array($result1)){
					$ResponseXML .= "<Com_rep><![CDATA[".$row_rscrn["Sal_ex"]."]]></Com_rep>";	
			//	}
			//} else {
...
//		echo $sql_rscrntrn;
        	...
        	while ($row_rscrntrn = mysql_fetch_array($result_rscrntrn)){
				...
				$sql_rssal = "select sdate,deli_date, GRAND_TOT, TOTPAY from s_salma where REF_NO = '" . trim($row_rscrntrn["Inv_no"]) . "'";
//				echo $sql_rssal;
        		...
        		$row_rssal = mysql_fetch_array($result_rssal);
         		if ((is_null($row_rssal["deli_date"])==false) and ($row_rssal["deli_date"]!="0000-00-00")) {
            		...
            		if ((is_null($row_rssal["sdate"])==false) and ($row_rssal["sdate"]!="0000-00-00")) { 
						$Inv_date  = $row_rssal["sdate"]; 
					}
         		}
				$balance_val=$row_rssal["GRAND_TOT"]-$row_rssal["TOTPAY"];
		 
				if ((is_null($row_rssal['TOTPAY'])==false) and ($row_rssal['TOTPAY']!="")){
					$Settle_amo=$row_rssal['TOTPAY'];
				} else {
					$Settle_amo=0;
				}
				...
				$sql_tmpinst= " insert into tmp_cash_credit_note_form (crn_form_no, Inv_date, inv_no, Amount, settleamt, settledate, days,  Incen_per, Incen_val, tmp_no, balance,disc1,invinv_date) values ('".$_GET["invno"]."', '".$Inv_date."', '".$row_rscrntrn["Inv_no"]."', '".$row_rscrntrn['Amount']."', ".$Settle_amo.", '".$row_rscrntrn["Mon"]."', ".$days.", ".$row_rscrntrn["Incen_per"].", ".$row_rscrntrn["Incen_val"].", '".$_SESSION["credit_note_form"]."', ".$balance_val.", '".$row_rscrntrn["disc1"]."','" . $row_rssal['sdate'] . "')";
//				echo $sql_tmpinst;
        		$result_tmpinst =$db->RunQuery($sql_tmpinst);
			...
			$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
							 <td width=\"110\"  background=\"\" ><font color=\"#FFFFFF\">Inv.Date</font></td>	
                              <td width=\"100\"  background=\"\" ><font color=\"#FFFFFF\">Deli.Date</font></td>
                              <td width=\"300\"  background=\"\"><font color=\"#FFFFFF\">Invoice No</font></td>
                              <td width=\"100\"  background=\"\"><font color=\"#FFFFFF\">Inv.Amount</font></td>
                              <td width=\"100\"  background=\"\"><font color=\"#FFFFFF\">Settle Amou</font></td>
        	...
        	while ($row = mysql_fetch_array($result)){
					...
					$ResponseXML .= "<tr>
							<td ><input type=\"text\" size=\"10\"  disabled  value='".$row['InvInv_date']."' class=\"text_purchase3\"/></td>
							 <td ><input type=\"text\" size=\"10\" name=".$Inv_date."  disabled id=".$Inv_date." value='".$row['Inv_date']."' class=\"text_purchase3\"/></td>
							 <td ><input type=\"text\" size=\"30\" name=".$inv_no."  disabled id=".$inv_no." value='".$row["inv_no"]."' class=\"text_purchase3\"/></td>
							 <td ><input type=\"text\" size=\"10\" name=".$Amount."  disabled id=".$Amount." value=".$row['Amount']." class=\"text_purchase3_right\"/></td>
...
if($_POST["Command"]=="save_item")
{
  ...
  if (($_POST["txt_cuscode"] != "") and ($_POST["mcou"] > 0)){
    $mrefno = trim($_POST["txtrefno"]);
...
//            echo "test1";
//            die();
            die("duplicate cash credit note detected");
...
//        die();
        ...
        while ($_POST["mcou"]>$i){
		  ...
		  if ($_POST[$Incen_val]!=""){	
			$sql1= "Insert into s_crnfrmtrn (Sdate, Refno, Code, Name, Sal_ex, Mon, Inv_no, Inv_date, Amount, Qty, Incen_per, Incen_val, Brand, Flag, tmp_no,disc1) values('" . $_POST["DTPicker1"] . "', '" . $mrefno . "','" . trim($_POST["txt_cuscode"]) . "', '" . trim($_POST["txt_cusname"]) . "', '" . $_POST["Com_rep"] . "', '" . $_POST[$settledate] . "','" . trim($_POST[$inv_no]) . "', '" . $_POST[$Inv_date] . "', '" . $_POST[$Amount] . "', '" . $_POST[$Qty] . "', '" . $_POST[$Incen_per] . "', '" . $_POST[$Incen_val] . "','" . $_POST[$Brands] . "','CCRN', '".$_SESSION["credit_note_form"]."','" . $_POST[$disc1] . "')";
			$result1=mysql_query($sql1, $dbinv);
			if ($result1!=1){ $sql_status=1; }	
			$mamount = $mamount + $_POST[$Incen_val];
		  }
		...
		$sql1= "insert into s_crnfrm (Refno, Sdate, Code, Mon, Amount, Remark, Sal_ex, Flag, Checked, Lock1, Cancell, Credit_note, tmp_no, company, Check_date) Values('" . $mrefno . "','" . $_POST["DTPicker1"] . "', '" . trim($_POST["txt_cuscode"]) . "', '" . $_POST["MonthView1"] . "', '" . $mamount . "','" . trim($_POST["txtremark"]) . "','" . $_POST["Com_rep"] . "','CCRN', '" . $_SESSION["CURRENT_USER"] . "', '0', '0', 'A', '".$_SESSION["credit_note_form"]."', '" . $_SESSION['company'] . "', '" . date("Y-m-d") . "')";
		$result1=mysql_query($sql1, $dbinv);
		if ($result1!=1){ $sql_status=1; }				
       
	    $sql1= "Update invpara set CCRNNO = CCRNNO+1 where COMCODE = '" .$_SESSION['COMCODE']. "'";
		$result1=mysql_query($sql1, $dbinv);
		if ($result1!=1){ $sql_status=1; }		
...
//        die();
        ...
        while ($_POST["mcou"]>$i){
			...
			$disc1="disc1".$i;
			if ($_POST[$Incen_val]!=""){	
                            $sql1= "Insert into s_crnfrmtrn (Sdate, Refno, Code, Name, Sal_ex, Mon, Inv_no, Inv_date, Amount, Qty, Incen_per, Incen_val, Brand, Flag, tmp_no,disc1) values('" . $_POST["DTPicker1"] . "','" . $mrefno . "','" . trim($_POST["txt_cuscode"]) . "','" . trim($_POST["txt_cuscode"]) . "','" . $_POST["Com_rep"] . "', '" . $_POST[$settledate] . "','" . trim($_POST[$inv_no]) . "','" . $_POST[$Inv_date] . "','" . $_POST[$Amount] . "', '" . $_POST[$Qty] . "', '" . $_POST[$Incen_per] . "', '" . $_POST[$Incen_val] . "','" . $_POST[$Brands] . "','CCRN', '".$_SESSION["credit_note_form"]."','" . $_POST[$disc1] . "')";
//                            echo "test3 $sql1";
//                            die();
                            $result1=mysql_query($sql1, $dbinv);
                            $mamount = $mamount + $_POST[$Incen_val];
