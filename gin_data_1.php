...
if ($_GET["Command"] == "new_inv") {
    ...
    if ($_SESSION["dev"] != "") {
        ...
        $sql = "Select GINS from invpara";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);
        $row = mysqli_fetch_array($result);
        $tmpinvno = "000000" . $row["GINS"];
        $lenth = strlen($tmpinvno);
        $invno = trim("GINS/ ") . substr($tmpinvno, $lenth - 8);
        $_SESSION["invno"] = $invno;

        $sql = "Select GINS from tmpinvpara";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);
        $row = mysqli_fetch_array($result);
        $_SESSION["tmp_no_gin"] = "GINS/" . $row["GINS"];

        $sql = "delete from tmp_gin_data where tmp_no='" . $_SESSION["tmp_no_gin"] . "'";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);

        $sql = "update tmpinvpara set GINS=GINS+1";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);
...
if ($_GET["Command"] == "save_item") {
    ...
    if ($_SESSION["tmp_no_gin"] != "") {
        ...
        $result = mysqli_query($GLOBALS['dbinv'], $sql);
        if ($row = mysqli_fetch_array($result)) {
            echo "GINS Number alredy exists";
        } else {
            ...
            $sql1 = "insert into s_ginmas_up(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '" . $_GET["to_dep"] . "', '" . $DESCRIPTION_to . "', '" . $_SESSION["tmp_no_gin"] . "')";
            //echo $sql1;
            $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
            ...
            while ($row2 = mysqli_fetch_array($result2)) {
                ...
                $sql1 = "insert into s_trn(STK_NO, SDATE, QTY, LEDINDI, REFNO, DEPARTMENT, seri_no, Dev, SAL_EX, ACTIVE, DONO) values ('" . $row2["str_code"] . "', '" . $_GET["invdate"] . "', '" . $cur_qty . "', 'TRN', '" . $invno . "', '" . $_GET["from_dep"] . "', '', '" . $_SESSION['dev'] . "', '', '1', '')";
                //echo $sql1;
                $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
                ...
                if ($rowsub = mysqli_fetch_array($resultsub)) {
                    ...
                    $sql1 = "update s_submas set QTYINHAND=" . $cur_qty . " where STK_NO='" . $row2["str_code"] . "' and STO_CODE='" . $_GET["to_dep"] . "'";
                    //echo $sql1;
                    $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
                } else {
                ...
                $sql1 = "update s_submas set QTYINHAND=0 where STK_NO = '" . $row2["str_code"] . "' and STO_CODE != '" . $_GET["to_dep"] . "'";
                //echo $sql1;
                $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);

                $sql1 = "update s_mas set QTYINHAND=" . $cur_qty . " where STK_NO='" . $row2["str_code"] . "'";
                //echo $sql1;
                $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
            }

            $sql1 = "update invpara set GINS=GINS+1";
            $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
