		...
		if ($row_sttr= mysql_fetch_array($result_sttr)){
			...
			while ($row_sttr= mysql_fetch_array($result_sttr)){
				...
				if ($apdays < $day1) {
                    ...
                    if ($row_inv["DEV"] == "0") { $commission = $cat1 * $row_sttr["ST_PAID"] * 0.01 / (1 + ($txtvat_new / 100)); }
                ...
                if (($apdays > $day1 - 1) and ($apdays < $day2)) {
                    if ($cat2 > 0) {
                        ...
                        if ($row_inv["DEV"] == "0") { $commission = $cat2 * $row_sttr["ST_PAID"] * 0.01 / (1 + ($txtvat_new / 100)); }
				...
				// }	
				 //	echo "(S4)";
                    ...
                    if ($row_inv["DEV"] == "0") { $commission = $cat3 * $row_sttr["ST_PAID"] * 0.01 / (1 + ($txtvat_new / 100)); }
...
if($_GET["Command"]=="view_summery")
{
	...
	while ($row = mysql_fetch_array($result)){
		...
		//echo $sql_st;
		$result_st1=mysql_query($sql_st1, $dbinv); 
	...
	//echo $sql;
		...
		while ($row_sttr = mysql_fetch_array($result_sttr)){
			...
			//}	
			//echo $sql_sttr1;
			$result_sttr1=mysql_query($sql_sttr1, $dbinv); 
	...
	$sql1="SELECT SUM(AMOUNT)AS RETAMU  FROM c_bal WHERE (flag1 = '0') and MONTH(SDATE)='".$month."' AND YEAR(SDATE) ='".$year."' AND SAL_EX='".$_GET["cmbrep"]."' AND CANCELL='0' and (trn_type='GRN' or  trn_type='CNT' ) and DEV='" . $_GET["cmbdev"] . "' ";
	//echo $sql1;
	$result1=mysql_query($sql1, $dbinv); 
...
$ResponseXML .= "<txtover60><![CDATA[".number_format($txtover60, 2, ".", ",")."]]></txtover60>";
	...
	if ($txtnett != "") {
		if ($year<=2014){
			...
			if ($txtnett != 0) { $TXTRATO = ($txtover60 + $TXTADJ + $txtretcheq) / ($txtnett * 1.11)*100; }
		}	
	}
