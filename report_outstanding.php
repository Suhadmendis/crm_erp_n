...
if ($_GET["radio2"]=="optdate") {
	...
	$sql="select * from s_salma where  dev!='" . $sysdiv . "' and  cancell='0' and SDATE<='" . $_GET["dtdate"] . "' and sdate >='2014-01-01'  order by id desc";
	//echo $sql;
	$sql_tmp ="";
	$result =$db->RunQuery($sql);
	while($row = mysql_fetch_array($result)){
			...
			if ($i!=0){
      		...
      		$sql_tmp =  $sql_tmp . " ('" . $row["REF_NO"] . "', '" . $row["SDATE"] . "','" . $row["GRAND_TOT"] . "' ," . $paid . ",'" . $row["C_CODE"] . "','" . $row["CUS_NAME"] . "')";
			$i=1;
			
   
	}
	
	 $ins = "insert into tmpsttr (refno, sdate, cheVal, paid,  code, cusname) values " . $sql_tmp;
		//echo $ins;
		$result_tmp =$db->RunQuery($ins);
		
	$i =0;
	$sql="select sum(st_paid) as paid,st_invono,sdate,c_code,CUS_NAME from view_salma_sttr where   cancell='0'   and SDATE<='" . $_GET["dtdate"] . "' and sdate >='2014-01-01' and st_date<='" . $_GET["dtdate"] . "' group by st_invono,sdate,c_code,CUS_NAME";
	 $sql_tmp ="";
	$result =$db->RunQuery($sql);
	while($row = mysql_fetch_array($result)){
			...
			if ($i!=0){
      		...
      		$sql_tmp =  $sql_tmp . " ('" . $row["ST_INVONO"] . "', '" . $row["SDATE"] . "','0' ," . $row["paid"] . ",'" . $row["C_CODE"] . "','" . $row["CUS_NAME"] . "')";
			$i=1;
	...
	echo "<center><table border=1><tr>
		...
		//echo $sql;
		 $sql="Select sdate,refno,code,cusname,sum(cheVal) as cheVal ,sum(paid) as paid,sum(cheVal-paid) as cheVal1 from tmpsttr group by sdate,refno,code,cusname having  cheVal1>0  order by code,sdate";
		$totbal=0;
		$balance=0;
		$i=0;
		$result =$db->RunQuery($sql);
		while($row = mysql_fetch_array($result)){
			...
			if (strtoupper($row["code"])!=strtoupper($ccode)){
				if ($i!=0){
					echo "<tr>
				...
				$rst_cus="select * from vendor where CODE='".$row["code"]."'";
				$result_cus =$db->RunQuery($rst_cus);
				$row_cus = mysql_fetch_array($result_cus);
	
				echo "<tr>
					<th colspan=9 align=left><b>".$row["code"]." ".$row_cus["NAME"]."</b></th>
					</tr>";
					$balance=$row["cheVal"]-$row["paid"];
			} else {
			...
			if (($row["sdate"]=="0000-00-00") or (is_null($row["sdate"])==true) or ($row["sdate"]=="")){
				echo "<td><font color=\"#FF0000\"></font></td>";
			} else {
				echo "<td><font color=\"#FF0000\">".$row["sdate"]."</font></td>";
			}	
			...
			echo "<td align=\"right\">".number_format($row["cheVal"], 2, ".", ",")."</td>
			<td align=\"right\">".number_format(($row["cheVal"]-$row["paid"]), 2, ".", ",")."</td>
			</tr>";
			$totbal=$totbal+$row["cheVal"]-$row["paid"];
		}
...
if ($_GET["radio2"]=="optdate1") { 
		...
		while ($row_inv = mysql_fetch_array($result_inv)){
		  $userd[] = "('".$row_inv["REF_NO"]."', '".$row_inv["NAME"]."', '".$row_inv["C_CODE"]."', ".$row_inv["GRAND_TOT"].", ".$row_inv["paid"].", '".$row_inv["SDATE"]."', '".$row_inv["SAL_EX"]."')";
			
			 
			
		}
		
		$sql_tmpinv="insert into tmpoutinv(REF_NO, Customer, Cus_Code, AMOUNT, paid, SDATE, sal_ex) values "  . implode(',', $userd);
		 $db->RunQuery($sql_tmpinv);
		...
		//echo $sql_inv;
		while ($row_inv = mysql_fetch_array($result_inv)){
			$userd1[] = "('".$row_inv["REF_NO"]."', '".$row_inv["NAME"]."', '".$row_inv["C_CODE"]."', ".$row_inv["GRAND_TOT"].", 0, '".$row_inv["SDATE"]."', '".$row_inv["SAL_EX"]."')";
			 
			
		}
		$sql_tmpinv="insert into tmpoutinv(REF_NO, Customer, Cus_Code, AMOUNT, paid, SDATE, sal_ex) values "  . implode(',', $userd1);
		 $db->RunQuery($sql_tmpinv);
		...
		while ($row_inv = mysql_fetch_array($result_inv)){
			$userd2[] = "('".$row_inv["REF_NO"]."', '".$row_inv["CUS_NAME"]."', '".$row_inv["C_CODE"]."', ".$row_inv["GRAND_TOT"].", 0, '".$row_inv["SDATE"]."', '".$row_inv["SAL_EX"]."')";		
		}
		 $sql_tmpinv="insert into tmpoutinv(REF_NO, Customer, Cus_Code, AMOUNT, paid, SDATE, sal_ex) values "  . implode(',', $userd2);
       $db->RunQuery($sql_tmpinv);
...
function currentrepinv()
{
	...
	$rep = $_GET["cmbrep"];
        ...
        if ($_SESSION["company"]!="C"){
            ...
            if ($_GET['chk_rec_c'] =="on") {
                $phrase = " company='C' and ";
            }
        }
            ...
            if ($_GET["chkpe"] != "on") {
                if ($_GET["cmbdev"] == "All"){
                   if ($_GET["cmbrep"] == "All") { 
				   		...
				   		$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpdate."'or SDATE='".$tmpdate."') and CANCELL='0'  order by C_CODE, SDATE";
						
					}	
                    
					if ($_GET["cmbrep"] != "All") { 
					
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep."'and (SDATE<'".$tmpdate."'or SDATE='".$tmpdate."')and CANCELL='0'  order by C_CODE, SDATE";
				...
				if ($_GET["cmbdev"] == "Computer") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpdate."'or SDATE='".$tmpdate."')and CANCELL='0' and DEV='0' order by C_CODE, SDATE";
                   	}
				   	if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep."'and (SDATE<'".$tmpdate."'or SDATE='".$tmpdate."')and CANCELL='0' and DEV='0' order by C_CODE, SDATE";
                	}
                }
                ...
                if ($_GET["cmbdev"] == "Manual") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where  $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" .$tmpdate. "'or SDATE='".$tmpdate."')and CANCELL='0' and DEV='1' order by C_CODE, SDATE";
					}	
                   	if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase  GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep. "'and (SDATE<'".$tmpdate."'or SDATE='".$tmpdate."')and CANCELL='0' and DEV='1'order by C_CODE, SDATE";
                	}
            	}
            }
			...
			if ($_GET["chkpe"] == "on") {
                if ($_GET["cmbdev"] == "All") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpover."'or SDATE='".$tmpover."')and (SDATE>'".$tmpbellow."'or SDATE='".$tmpbellow."')  and CANCELL='0' order by C_CODE, SDATE";
					}	
                    
					if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep. "'and (SDATE<'".$tmpover."'or SDATE='".$tmpover."')and (SDATE>'".$tmpbellow."' or SDATE='".$tmpbellow."') and CANCELL='0' order by C_CODE, SDATE";
					}	
               }
               ...
               if ($_GET["cmbdev"] == "Computer") {
                   if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpover ."' or SDATE='".$tmpover."') and (SDATE>'".$tmpbellow."' or SDATE='".$tmpbellow."') and CANCELL='0' and DEV='0' order by C_CODE, SDATE";
					}	
                   	
					if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep."' and (SDATE<'".$tmpover."'or SDATE='".$tmpover."')and (SDATE>'".$tmpbellow."' or SDATE='".$tmpbellow."') and CANCELL='0' and DEV='0' order by C_CODE, SDATE";
               		}
				}
				...
				if ($_GET["cmbdev"] == "Manual") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpover."' or SDATE='".$tmpover."') and (SDATE>'".$tmpbellow."' or SDATE='".$tmpbellow."') and CANCELL='0' and DEV='1' order by C_CODE, SDATE";
					}	
                    
					if ($_GET["cmbrep"] != "All") { 
						//echo "12";
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='".$rep."' and (SDATE<'".$tmpover."'or SDATE='".$tmpover."')and (SDATE>'".$tmpbellow."' or SDATE='".$tmpbellow."') and CANCELL='0' and DEV='1' order by C_CODE, SDATE";
                	}
            	}	
			}
    }    
            ...
            if ($_GET["chkpe"] != "on") {
                if ($_GET["cmbdev"] == "All") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'".$tmpdate ."' or SDATE='".$tmpdate."') and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'  order by C_CODE, SDATE";
                   	}
					if ($_GET["cmbrep"] != "All") { 
				   		
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpdate . "'or SDATE='" . $tmpdate . "')and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'  order by C_CODE, SDATE";
                	}
               }
			   ...
			   if ($_GET["cmbdev"] == "Computer") {
                    if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" . $tmpdate . "'or SDATE='" . $tmpdate . "')and CANCELL='0' and DEV='0' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                    if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpdate . "'or SDATE='" . $tmpdate . "')and CANCELL='0' and DEV='0' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
                	}
				}
                ...
                if ($_GET["cmbdev"] == "Manual") {
				   ...
				   if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" . $tmpdate . "'or SDATE='" . $tmpdate . "') and CANCELL='0' and DEV='1' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
				   }	
                   if ($_GET["cmbrep"] != "All") { 
				   		
						$sql = "SELECT * from view_salma_vendor where  $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpdate . "'or SDATE='" . $tmpdate . "')and CANCELL='0' and DEV='1' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
                   }
            	}
            }
			...
			if ($_GET["chkpe"] == "on") {
                if ($_GET["cmbdev"] == "All") {
                   if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE>'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "')  and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                    
					if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE<'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "') and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                }
                ...
                if ($_GET["cmbdev"] == "Computer") {
                   	if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE>'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "') and CANCELL='0' and DEV='0'and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                    
					if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE<'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "') and CANCELL='0' and DEV='0' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                }
                ...
                if ($_GET["cmbdev"] == "Manual") {
                   if ($_GET["cmbrep"] == "All") { 
						...
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE>'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "') and CANCELL='0' and DEV='1' and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                    if ($_GET["cmbrep"] != "All") { 
						
						$sql = "SELECT * from view_salma_vendor where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and (SDATE<'" . $tmpover . "'or SDATE='" . $tmpover . "')and (SDATE<'" . $tmpbellow . "'or SDATE='" . $tmpbellow . "') and CANCELL='0' and DEV='1'  and Brand='" . $_GET["cmbbrand"] . "' order by C_CODE, SDATE";
					}	
                }
            }
 	}
...
//  echo $sql;
   ...
   $row_head = mysql_fetch_array($result_head);
	...
	if ($_GET["cmbbrand"] == "All") {
    	if ($_GET["cmbdev"] == "All") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and CANCELL='0'";
			}	
        	
			if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "'  and CANCELL='0'";
			}	
    	}
    
    	if ($_GET["cmbdev"] == "Manual") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and DEV='1'  and CANCELL='0' ";
			}	
        	
			if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "' and DEV='1'  and CANCELL='0' ";
			}
		}		
        
    	if ($_GET["cmbdev"] == "Computer") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and DEV='0'  and CANCELL='0' ";
			}	
        	
			if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "' and DEV='0'  and CANCELL='0'";
			}	
    	}
	}
   ...
   $date=date("Y-m-d");
	...
	$tmpdate=date('Y-m-d', strtotime($date.$caldays));
   	...
   	if ($_GET["cmbbrand"] != "All"){
    	if ($_GET["cmbdev"] == "All") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        	
			if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
    	}
    
    	if ($_GET["cmbdev"] == "Manual") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and DEV='1'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
        	if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "' and DEV='1'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
    	}
    
    	if ($_GET["cmbdev"] == "Computer") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and DEV='0'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
        	if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as over120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE<'" . $tmpdate . "' and SAL_EX='" . $rep . "' and DEV='0'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
    	}
	}
   ...
   if ($_GET["cmbbrand"] == "All") {
   		if ($_GET["cmbdev"] == "All") {
        	if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "'  and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "'  and CANCELL='0'";
			}
        }
        
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='0'  and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "' and DEV='0'  and CANCELL='0' ";
			}	
        }
        
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='1' and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "'and DEV='1'  and CANCELL='0'";
			}	
        }
	}
   ...
   if ($_GET["cmbbrand"] != "All") {
        if ($_GET["cmbdev"] == "All") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "'  and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        }
        
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='0'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "' and DEV='0'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        }
        
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='1' and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t90to120 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "'and DEV='1'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        }
	}	
	...
	if ($_GET["cmbbrand"] == "All") {
        if ($_GET["cmbdev"] == "All") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "'  and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") {
			 	$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . $rep . "'  and CANCELL='0'";
			}
        }
        
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='0'  and CANCELL='0'";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . rep . "' and DEV='0'  and CANCELL='0' ";
			}	
        }
        
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='1'  and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . rep . "' and DEV='1'   and CANCELL='0'";
			}	
        }
	}
	...
	if ($_GET["cmbbrand"] != "All") {
        if ($_GET["cmbdev"] == "All") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . rep . "'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        }
        
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase  GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='0'  and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . rep . "' and DEV='0'  and CANCELL='0'   and Brand='" . $_GET["cmbbrand"] . "'";
			}	
        }
        
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and DEV='1'  and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "' ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$rst1= "SELECT sum(GRAND_TOT)-sum(totpay) as t60to90 from s_salma where $phrase GRAND_TOT > TOTPAY and SDATE>'" . $tmpdate . "' and  SDATE<'" . $tmpdate2 . "' and SAL_EX='" . rep . "' and DEV='1'   and CANCELL='0'  and Brand='" . $_GET["cmbbrand"] . "'";
        	}
		}
	}	
...
function currentrepcus()
{
	...
	if ($_GET["cmbbrand"] == "All") {
        if ($_GET["cmbdev"] == "All") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' group by C_CODE ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' group by C_CODE";
			}
        }
         
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' and DEV='1'  group by  C_CODE ";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' and DEV='1' group by C_CODE";
			}	
        }
         
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' and DEV='0'  group by C_CODE";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' and DEV='0'  group by C_CODE";
			}	
        }
	}
	...
	if ($_GET["cmbbrand"] != "All") {
        if ($_GET["cmbdev"] == "All") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'   group by C_CODE" ;
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' and Brand='" . $_GET["cmbbrand"] . "'  group by C_CODE ";
        	}
		}
         
        if ($_GET["cmbdev"] == "Manual") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' and DEV='1' and Brand='" . $_GET["cmbbrand"] . "'   group by C_CODE";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' and DEV='1' and Brand='" . $_GET["cmbbrand"] . "'  group by C_CODE";
			}	
        }
         
        if ($_GET["cmbdev"] == "Computer") {
            if ($_GET["cmbrep"] == "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and CANCELL='0' and DEV='0' and Brand='" . $_GET["cmbbrand"] . "' group by C_CODE";
			}	
            if ($_GET["cmbrep"] != "All") { 
				$sql = "SELECT sum(grand_tot) as GRAND_TOT, sum(totpay) as TOTPAY, C_CODE from s_salma where $phrase GRAND_TOT - TOTPAY >0 and SAL_EX='" . $rep . "'and CANCELL='0' and DEV='0'  and Brand='" . $_GET["cmbbrand"] . "'   group by C_CODE";
			}	
        }
	}
    	...
    	if ($_GET["chk_svat"] != "on") {
        	if ($_GET["chkcus"]=="on") {
            	...
            	if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            	if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."' and C_CODE='".$_GET["txt_cuscode"]."'and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) {
				 	$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) {
				 	$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."'and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by REF_NO";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
        	} else {
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."'and DEV!='".$GLOBALS[$sysdiv]."'and CANCELL='0'  order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and SAL_EX='".$_GET["cmbrep"]."'and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by REF_NO";
				}
					
            	if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and SAL_EX='".$_GET["cmbrep"]."'and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by id";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]. "'and SAL_EX='".$_GET["cmbrep"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0'  order by REF_NO";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) {
				 	$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and SAL_EX='".$_GET["cmbrep"]."'and CANCELL='0' order by id";
				}	
        	}
    	} else {
        	if ($_GET["chkcus"]=="on") {
				...
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0'  order by id";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) {
				 	$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."'and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) {
				 	$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0'  order by id";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."'and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and SAL_EX='".$_GET["cmbrep"]."' and C_CODE='".$_GET["txt_cuscode"]."' and DEV!='".$GLOBALS[$sysdiv]."'and CANCELL='0' and SVAT > '0' order by id";
        		}
				
			} else {
            
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) {
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"] . "' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]."'and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            	
				if (($_GET["cmbrep"] == "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            	
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optdaily")) {
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and SAL_EX='".$_GET["cmbrep"]."'and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] == "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."') and SAL_EX='".$_GET["cmbrep"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by id";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optdaily")) { 
					$sql = "SELECT * from s_salma where  SDATE='".$_GET["dtfrom"]."' and Brand='".$_GET["CmbBrand"]. "'and SAL_EX='".$_GET["cmbrep"]."' and DEV!='".$GLOBALS[$sysdiv]."' and CANCELL='0' and SVAT > '0' order by REF_NO";
				}	
            
				if (($_GET["cmbrep"] != "All") and ($_GET["CmbBrand"] != "All") and ($_GET["radio2"]=="optperiod")) { 
					$sql = "SELECT * from s_salma where  (SDATE>'".$_GET["dtfrom"]."' or SDATE='".$_GET["dtfrom"]."') and (SDATE<'".$_GET["dtto"]."' or SDATE='".$_GET["dtto"]."')  and Brand='".$_GET["CmbBrand"]."' and SAL_EX='".$_GET["cmbrep"]."' and CANCELL='0' and SVAT > '0' order by id";
        		}
			}	
    	}
...
function summeryrep()
{
	...
	if ($_GET["radio2"]=="optdaily") { 
		$sql_sale = "select * from s_salma where  SDATE='".$_GET["dtfrom"]."' and DEV!='".$dev."' and CANCELL='0' ";
		
		if ($_GET["radio2"]=="optperiod") { 
			$sql_sale = "select * from s_salma where  ( SDATE='".$_GET["dtfrom"]."' OR  SDATE>'".$_GET["dtfrom"]."' ) AND ( SDATE='".$_GET["dtto"]."'  OR  SDATE<'".$_GET["dtto"]."' )and DEV!='".$dev."' and CANCELL='0'";
		}	
	...
	while ($row_c_bal = mysql_fetch_array($result_c_bal)){
		...
		if ($row_crnma = mysql_fetch_array($result_crnma)){
        	$sql_salma = " Select * from s_salma where  REF_NO = '" . $row_crnma["INVOICENO"] . "' ";
...
function summerysprte()
{
	...
	while ($row_brand = mysql_fetch_array($result_brand)){
		...
		if ($_GET["chkcus"] == "on") {
      		if ($_GET["cmbrep"] == "All") {
        		if ($_GET["radio2"]=="optdaily") { 
					$sql_sale = "select * from s_salma where  C_CODE ='".trim($_GET["txt_cuscode"]) . "' and SDATE='" . $_GET["dtfrom"] . "' and brand='" . $row_brand["barnd_name"] . "' and DEV!='" . $GLOBALS[$sysdiv] . "' and CANCELL='0' ";
				}	
        		if ($_GET["radio2"]=="optperiod") { 
					$sql_sale = "select * from s_salma where  c_code ='" . trim($_GET["txt_cuscode"]) . "' and ( SDATE='" . $_GET["dtfrom"] . "' OR  SDATE>'" . $_GET["dtfrom"] . "' ) AND ( SDATE='" . $_GET["dtto"] . "'  OR  SDATE<'" . $_GET["dtto"] . "' )and brand='" . $row_brand["barnd_name"] . "' and DEV!='" . $GLOBALS[$sysdiv] . "'and CANCELL='0'  ";
      			}
      		}	
			
			if ($_GET["cmbrep"] != "All") {
        		if ($_GET["radio2"]=="optdaily") { 
					$sql_sale = "select * from s_salma where  c_code ='" . trim($_GET["txt_cuscode"]) . "' and sal_ex='" . trim($_GET["cmbrep"]) . "' and  SDATE='" . $_GET["dtfrom"] . "' and brand='" . $row_brand["barnd_name"] . "' and DEV!='" . $GLOBALS[$sysdiv] . "' and CANCELL='0' ";
				}	
        		
				if ($_GET["radio2"]=="optperiod") {  
					$sql_sale = "select * from s_salma where  c_code ='" . trim($_GET["txt_cuscode"]) . "' and sal_ex='" . trim($_GET["cmbrep"]) . "' and ( SDATE='" . $_GET["dtfrom"] . "' OR  SDATE>'" . $_GET["dtfrom"] . "' ) AND ( SDATE='" . $_GET["dtto"] . "'  OR  SDATE<'" . $_GET["dtto"] . "' )and brand='" . $row_brand["barnd_name"] . "' and DEV!='" . $GLOBALS[$sysdiv] . "'and CANCELL='0'  ";
				}	
      		}
		}
