						...
						if ($_GET["Option2"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								/*if ($_GET["stname"]=="ord"){
									...
									$sql = mysql_query("SELECT * from s_cusordmas where REF_NO like  '$letters%' and  INVNO='0' and CANCELL='0'  and Result='OK' order by SDATE desc limit 50") or die(mysql_error());
								//}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
							/*	if ($_GET["stname"]=="ord"){
									...
									$sql = mysql_query("SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' and Result='OK'  order by SDATE desc limit 50") or die(mysql_error()) or die(mysql_error());
								//}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
...
if ($_GET["Command"]=="pass_invno_cus_ord"){
	...
	// Call setcus
        ...
        while($row_rsinv = mysql_fetch_array($sql_rsinv)){
			...
			('".$_GET["invno"]."', '".$row_rsinv["STK_NO"]."', '".$row_rsinv["DESCRIPT"]."', ".$PRICE.", ".$row_rsinv["QTY"].", ".$row_rsinv["DIS_per"].", ".$row_rsinv["DIS_rs"].", ".($row_rsinv["QTY"]*$PRICE).", '".$row_rsinv["STK_NO"]."', '".$row_rsinv["STK_NO"]."')") or die(mysql_error());	
...
FROM         brand_mas RIGHT OUTER JOIN
				...
				if ($NewRefNo==$salesrep){
					$sqlinvcheq = mysql_query("SELECT * FROM s_invcheq WHERE che_date>'".$_GET["invdate"]."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and (sal_ex='".$OldRefno."' or sal_ex='".trim($salesrep)."'") or die(mysql_error());
				} else {	
		...
		if ($_SESSION['company']=="THT"){	
			...
			//('".$_GET['invno']."', '".$_GET['itemcode']."', '".$_GET['item']."', ".$rate.", ".$qty.", ".$_GET["discountper"].", ".$discount.", ".$subtotal.", '".$_GET["brand"]."', '".$_SESSION["tmp_no_ord"]."', '".$_GET["str_partno"]."') ";
				...
				//	echo "Insert into tmp_inv_data(str_invno, str_code, str_description, cur_rate, cur_qty, dis_per,cur_discount, cur_subtot, brand, actual_selling, ad, tmp_no) values ( '".$_GET["newinvno"]."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', ".$SELLING.", ".$row['QTY'].", ".$row['DIS_per'].", ".$dis_val.", ".(($SELLING*$row['QTY'])-$dis_val).", '".$row['BRAND']."', ".$rowit["SELLING"].", '".$row["ad"]."', '".$_SESSION["tmp_no_salinv"]."')";
					$sql_tmp = mysql_query("Insert into tmp_inv_data(str_invno, str_code, str_description, cur_rate, cur_qty, dis_per,cur_discount, cur_subtot, brand, actual_selling, ad, tmp_no) values ( '".$_GET["newinvno"]."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', ".$SELLING.", ".$row['QTY'].", ".$row['DIS_per'].", ".$dis_val.", ".(($SELLING*$row['QTY'])-$dis_val).", '".$row['BRAND']."', ".$rowit["SELLING"].", '".$row["ad"]."', '".$_SESSION["tmp_no_salinv"]."')") or die(mysql_error());
				}
