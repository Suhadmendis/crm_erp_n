...
if ($_GET["Command"] == "getdt") {
    ...
    $sql = "SELECT userdepcode FROM invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    //$no = $row['vmrcode'];
    $post = generateId($row['userdepcode'], "UDPT/", "post");
    $uniq = uniqid();
...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        $sql = "delete from user_department where ref = '" . $_GET['ref'] . "'";
        $result = $conn->query($sql);
        $sql = "Insert into user_department(ref,userdept,user)values 
           ('" . $_GET['ref'] . "','" . $_GET['userdept'] . "','" . $_GET['user'] . "')";
        ...
        $no = $row['userdepcode'];
        $no2 = $no + 1;
        $sql = "update invpara set userdepcode = $no2 where userdepcode = $no";
        $result = $conn->query($sql);
