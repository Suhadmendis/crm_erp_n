...
//            $sql = "select jobCost,COST,rawWaste from s_mas where STK_NO = '" . $row["stk_no"] . "'";
        ...
        $sql = "insert into s_salma (REF_NO,SDATE,trn_type,  C_CODE, CUS_NAME,c_add1, VAT_VAL, SVAT, vat, tmp_no,REMARK,btt,grand_tot,SAL_EX,gst,btt_rate,use_name,DEPARTMENT,dele_no,ORD_NO,JOB_REF,Customer_PO,currency,rate,advance,vendor_code,C_NBT,C_VAT,C_SVAT) values 
	('" . $invno . "', '" . $_GET['invdate'] . "','INV' ,'" . $_GET["customercode"] . "', '" . $_GET["customername"] . "','" . $_GET["cont_p"] . "','" . $mtot1 . "','" . $svatValue . "', '$vatType', '" . $_GET["tmpno"] . "','" . $_GET['txt_remarks'] . "','" . $nbt . "','" . $mgrand_tot . "','" . $_GET['salesrep'] . "','" . $row['vatrate'] . "', '" . $row['nbt'] . "', '" . $_SESSION['UserName'] . "','" . $_GET['department'] . "','" . $_GET['DANO'] . "','" . $_GET['txt_minno'] . "','" . $_GET['txt_job'] . "','" . $_GET['Customer_PO'] . "','" . $_GET['currency'] . "','" . $_GET['rate'] . "','" . $_GET['advance'] . "','" . $_GET['vendor_code'] . "','" . $_GET['C_NBT'] . "','" . $_GET['C_VAT'] . "','" . $_GET['C_SVAT'] . "')";
       // echo $sql;
        $result = $conn->query($sql);
        ...
        if($mtot1>0){
            ...
            $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, L_LMEM, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . $invno . "', '" . $_GET["invdate"] . "', '" . $VAT_LCODE . "', '$VAT_LNAME', " . $vatAmount . ", 'INV', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "','" . $_GET["customercode"] . "')";
        ...
        if($vatAmount > 0){
            ...
            $ledgerDes = $rowacc["C_NAME"];

            $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, L_LMEM, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . $invno . "', '" . $_GET["invdate"] . "', '" . $rowacc['C_CODE'] . "', '$ledgerDes', " . $vatAmount . ", 'INV', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "','" . $_GET["customercode"] . "')";
            $conn->query($sqlLedger);
        ...
        $vatAmount = number_format($vatAmount, 2, ".", "");
         ...
         $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='12201001-000'";
            $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $vatAmount = number_format($vatAmount, 2, ".", "");
            ...
            $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='51101002-000'";
            $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $sqlacc = "select L_CODE,L_AMOUNT from ledger where L_REFNO= '" . $_GET["txt_fg"] . "' and L_FLAG1 = 'DEB'";
        $rowacctemp = $conn->query($sqlacc)->fetch();
      

        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE= '" . $rowacctemp["L_CODE"] . "'";
        $rowacc = $conn->query($sqlacc)->fetch();


        $ledgerDes = $rowacc["C_NAME"];
       
        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, L_LMEM, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . $invno . "', '" . $_GET["invdate"] . "', '" . $rowacc["C_CODE"] . "', '$ledgerDes', " . $glAmount. ", 'INV', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "','" . $_GET["customercode"] . "')";
        $conn->query($sqlLedger);
        ...
        if ($_GET["Command1"] == "getGl") {
            ...
            $msg .= "<td style = 'width: 120px;text-align: left;'>" . $rowacc["C_NAME"] . "</td>";
            $msg .= "<td style = 'width: 10px;text-align: left;'></td>";
            $msg .= "<td style = 'width: 10px;text-align: left;'>" . number_format($rowacctemp["L_AMOUNT"], 2, ".", ",") . "</td>";
            $msg .= "</tr>";
        }
        ...
        // $sqlacc = "select L_CODE,L_AMOUNT from ledger where L_REFNO= '" . $_GET["txt_fg"] . "' and L_FLAG1 = 'CRE'";
        // $rowacctemp = $conn->query($sqlacc)->fetch();
      
        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE= '22401033-000'";
        $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $glAmount = $rawConsumpt + $rawWaste;
            ...
            $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='44201001-000'";
            $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $costing_ref = $_GET["txt_costing"];
        
        $sqlcosting = "select * from costing_cal where Cost_fer = '$costing_ref'";
        $result = $conn->query($sqlcosting);
        $rowcostingCAL = $result->fetch();

        $glAmount = $rowcostingCAL['L_UC']*$main_qty;
        $L1 = $glAmount;
        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='45201001-000'";
        $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $glAmount = $rowcostingCAL['V_UC']*$main_qty;
        $L2 = $glAmount;
         $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='45201002-000'";
            $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $glAmount = str_replace(",","",$rowcostingCAL['F_UC'])*$main_qty;

        $F3 = $glAmount;
       
        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='45201003-000'";
        $rowacc = $conn->query($sqlacc)->fetch();
        ...
        $glAmount = $jobCostDl + $jobCostVar + $jobCostFx;
          ...
          $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='45201004-000'";
            $rowacc = $conn->query($sqlacc)->fetch();
