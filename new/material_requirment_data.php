...
if ($_GET["Command"] == "getdt") {
    ...
    $sql = "SELECT material_requirement_code FROM invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    $no = $row['material_requirement_code'];
//    uniq
    $uniq = uniqid();

    $tmpinvno = "0000000" . $row["material_requirement_code"];
    $lenth = strlen($tmpinvno);
    $no = trim("MR/") . substr($tmpinvno, $lenth - 7);
...
//add
        ...
        $no = $row['material_requirement_code'];
        $uniq = uniqid();
        $tmpinvno = "000000000" . $row["material_requirement_code"];
        $lenth = strlen($tmpinvno);
        $no = trim("MR/") . substr($tmpinvno, $lenth - 7);
...
//        $result = $conn->query($sql);
        ...
        $sql1 = "Insert into material_requirement(reference_no,customer_m,customer_f,date,description,handled,order_quantity,remarks,prepaired_by,user,uniq)values 
                        ('" . $no . "','" . $_GET['customer_m'] . "','" . $_GET['customer_f'] . "','" . $_GET['date'] . "','" . $_GET['description'] . "','" . $_GET['handled'] ."','" . $_GET['order_quantity'] ."','" . $_GET['remarks'] . "','" . $_GET['prepaired_by'] ."','" . $_GET['user'] . "','" . $_GET['uniq'] . "')";
        $result = $conn->query($sql1);
                
        $sql2 = "SELECT * FROM material_requirement_table_temp where uniq = '" . $_GET['uniq'] . "'";


        foreach ($conn->query($sql2) as $row) {

            $sql = "Insert into material_requirement_table(reference_no,no,code,des,uom,cost,outs,qty,uniq)values 
             ('" . $no . "','" .$row['no'] . "','" . $row['code'] . "','" . $row['des'] . "','" . $row['uom'] ."','" . $row['cost'] ."','" . $row['outs'] ."','" . $row['qty'] ."','" . $row['uniq'] . "')";
        ...
        foreach ($conn->query($sql2) as $row) {
            ...
            $sql = "DELETE FROM material_requirement_table_temp where uniq = '" . $_GET['uniq'] . "'";
        ...
        $no = $row['material_requirement_code'];
        $no2 = $no + 1;
        $sql = "update invpara set material_requirement_code = $no2 where material_requirement_code = $no";
        $result = $conn->query($sql);
...
//        $mid = (int) $pieces[1];
       ...
       if ($_GET["Command1"] == "add_tmp") {
        ...
        $sql = "Insert into material_requirement_table_temp(reference_no,no,code,des,uom,cost,outs,qty,uniq)values 
     ('" . $_GET['reference_no'] . "','" . $_GET['no'] . "','" . $_GET['code'] . "','" . $_GET['des'] . "','" . $_GET['uom'] . "','" . $_GET['cost'] . "','" . $_GET['outs'] ."','" . $_GET['qty'] ."','" . $_GET['uniq'] . "')";
    ...
    $ResponseXML .= "<sales_table><![CDATA[<table id='myTable' class='table table-bordered'>
     ...
     $sql1 = "SELECT * FROM material_requirement_table_temp where uniq = '" . $_GET['uniq'] . "'";


    foreach ($conn->query($sql1) as $row2) {

        $ResponseXML .= "<tr><td>" . $row2['no'] . "</td><td>" . $row2['code'] . "</td><td>" . $row2['des'] . "</td><td>" . $row2['uom'] . "</td><td>" . $row2['cost'] . "</td><td>" . $row2['outs'] . "</td><td>" . $row2['qty'] . "</td><td><a onclick='remove_tmp(" . $row2['id'] . ");' class='btn btn-default btn-sm'><span class=''></span> &nbsp; REMOVE</a></td></tr>";
    }
...
if ($_GET["Command"] == "removerow") {
    ...
    $sql = "delete from material_requirement_table_temp where id = '" . $_GET['id'] . "'";
    $result = $conn->query($sql);
    $ResponseXML .= "<sales_table><![CDATA[<table id='myTable' class='table table-bordered'>
                            <thead>
                               <tr>
