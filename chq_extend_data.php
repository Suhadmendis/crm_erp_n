...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		$sql_rs="insert into s_cheque_extend (refno, sdate, sal_ex, c_code, c_name, ch_no, ch_amount, ch_date, ch_exdate, approved, acc_approved, e_time) values ('".trim($_GET["txtrefno"])."', '".$_GET["txtsdate"]."', '".trim($_GET["txtsal_ex"])."', '".trim($_GET["txtcode"])."', '".trim($_GET["txtname"])."', '".trim($_GET["txtch_no"])."', ".trim($_GET["txtch_amount"]).", '".trim($_GET["txtch_date"])."', '".trim($_GET["txtchexdate"])."', '0', '0', '".date("H:i:s")."')";
		//echo $sql_rs;
		$result_rs =$db->RunQuery($sql_rs);
...
if($_GET["Command"]=="apprive")
{
	...
	$sql1="update s_cheque_extend set ch_exdate='".$_GET["txtchexdate"]."', approved='" . $app . "'  where refno='" . trim($_GET["txtrefno"]) . "'";
	$result1 =$db->RunQuery($sql1);
...
if($_GET["Command"]=="acc_apprive")
{
	...
	$sql="select * from s_cheque_extend where refno='" . trim($_GET["txtrefno"]) . "'";
	$result =$db->RunQuery($sql);
	if($row_rs = mysql_fetch_array($result)){
