...
//         str_code, str_description, cur_qty, str_invno
   ...
   $ResponseXML .= "<sales_table><![CDATA[<table class='table table-bordered' id='myTable'>
    ...
    foreach ($conn->query($sql) as $row) {
         ...
         $sqlsmas = "Select * from s_rawmas where STK_NO =  '" . $row['str_code'] . "'";
        $result = $conn->query($sqlsmas);
        $rowmas = $result->fetch();
...
$qty = $qty + $row['cur_qty'];
    ...
    $ResponseXML .= "<qty><![CDATA[" . number_format($qty) . "]]></qty>";
    $ResponseXML .= "</salesdetails>";
...
//        exit ($sql);
        ...
        $sql1 = "insert into s_mrnmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE, ISSUE, Manuel_NO, pro_des) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '" . $_GET["to_dep"] . "', '" . $DESCRIPTION_to . "', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'MRNG', '" . $_GET["issue"] . "', '" . $_GET["manuel_ref"] . "', '" . $_GET["Product"] . "')";
...
if ($_GET["Command"] == "update_list") {
    ...
    $sql = "select * from s_mrnmas where WHERE GROUP1 = 'PRE_INK' and REF_NO <> ''";
