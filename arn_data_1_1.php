...
if ($_GET["Command"] == "new_inv") {
    ...
    $sql = "Select ARN_2 from invpara where COMCODE='" . $_SESSION['company'] . "'";

    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    $tmpinvno = "000000" . $row["ARN_2"];
    $lenth = strlen($tmpinvno);
    $invno = $_SESSION['company'] . trim("2GA") . substr($tmpinvno, $lenth - 8);
...
if ($_GET["Command"] == "arn") {
    ...
    $sql = "select * from s_purmas_tmp_2 where REFNO='" . $_GET['invno'] . "' ";
//    echo $sql;
    //$ResponseXML .= $sql;
    $result = $db->RunQuery($sql);
    if ($row = mysql_fetch_array($result)) {
    ...
    $i = 1;
    $total = 0;
    $sql = "Select * from s_purtrn_tmp_2 where REFNO='" . $_GET['invno'] . "'";
    $result = $db->RunQuery($sql);
    while ($row = mysql_fetch_array($result)) {
...
if ($_POST["Command"] == "save_item") {
    ...
    if ($_POST["count"] > 0) {
        ...
        $sql = "select * from s_purmas_2 where REFNO='" . $_POST["invno"] . "'";
	  ...
	  ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST["invno"] . "', " . $_POST["total_value"] . ", " . $_POST["total_value"] . ", " . (-1 * $_POST["total_value"]) . ", '" . $_POST["supplier_code"] . "', '" . $supplier_name . "', '" . $_SESSION['dev'] . "', 0, 0, 0, '0', 'O', 'ARN', 'OVER')";
		   ...
		   if ($result1==false){ $sql_status=2; }
            ...
            $sql1 = "insert into s_purmas_2(REFNO, SDATE, ORDNO, LCNO, pi_no, COUNTRY, SUP_CODE, SUP_NAME, REMARK, DEPARTMENT, AMOUNT, PUR_DATE,
...
//                $sql4 = "insert into s_purtrn_2(REFNO, SDATE, STK_NO, DESCRIPT, COST, MARGIN, SELLING, REC_QTY, FOB, DEPARTMENT, QTYINHAND, O_QTY, 
// Cost_c, acc_cost, acc_cost_c, brand, vatrate, DISCOUNT, DISCOUNT2, ret_qty, cost_seling, cost_margin, CANCEL, soldqty, days) values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST[$itemcode_name] . "', '" . $_POST[$itemname_name] . "', " . $cost . ", " . $margin_name . ", " . $_POST[$selling_name] . ", " . $_POST[$qty_name] . ", " . $fob_val . ", '" . $_POST["department"] . "', " . $QTYINHAND . ", " . $_POST[$ord_qty_name] . ", " . $COST_mas . ", '" . $acc_cost . "', '" . $acc_cost_c . "', '" . $_POST["brand"] . "', '" . $vatrate . "', ".$_POST[$discount_a_name].", ".$_POST[$discount_b_name].", 0, 0, 0, '0', '', 0)";
                $sql4 = "insert into s_purtrn_2(REFNO, SDATE, STK_NO, DESCRIPT, COST, MARGIN, REC_QTY, FOB, DEPARTMENT, QTYINHAND, 
...
//                                die();
             ...
             //   }
               ...
               if ($row3 = mysql_fetch_array($result3)) {
                ...
                $sql1 = "update s_purmas_tmp_2 set CANCEL='2' where REFNO='" . $_POST["orderno1"] . "'";
				$result1=mysql_query($sql1, $dbinv);
				if ($result1==false){ $sql_status=9; }
               

                $sql1 = "update s_purtrn_tmp_2 set CANCEL='2' where REFNO='" . $_POST["orderno1"] . "'";
				$result1=mysql_query($sql1, $dbinv);
		...
		if ($sql_status==0){
        	$sql1 = "update invpara set ARN_2=ARN_2+1 where COMCODE='" . $_SESSION['company'] . "'";
			$result1=mysql_query($sql1, $dbinv);
...
if ($_GET["Command"] == "pass_arnno") {
    $ResponseXML = "";
    $ResponseXML .= "<salesdetails>";
    $sql = "Select * from s_purmas_2 where REFNO='" . $_GET['arnno'] . "'";
    $result = $db->RunQuery($sql);
    if ($row = mysql_fetch_array($result)) {
        $ResponseXML .= "<REFNO><![CDATA[" . $row["REFNO"] . "]]></REFNO>";
        $ResponseXML .= "<SDATE><![CDATA[" . $row["SDATE"] . "]]></SDATE>";
        ...
        $mcou = 0;
        $sql = "Select count(*) as mcou from s_purtrn_2 where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        $row = mysql_fetch_array($result);
        $mcou = $row["mcou"] + 1;
        ...
        $i = 1;
        $tot = 0;
        $sql = "Select * from s_purtrn_2 where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        while ($row = mysql_fetch_array($result)) {
...
if ($_GET["Command"] == "cancel_inv") {
    ...
    $sql = "select * from s_purmas_2 where CANCEL='0' order by id desc";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    //echo $sql;
    ...
    //} else {	
    $sql1 = "update s_purmas_2 set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_purtrn_2 set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_purmas_tmp_2 set cancel='0' where REFNO='" . $_GET["orderno1"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_purtrn_tmp_2 set CANCEL='0' where REFNO='" . $_GET["orderno1"] . "'";
    $result1 = $db->RunQuery($sql1);
    ...
    $sql1 = "select * from s_purtrn_2 where REFNO='" . $_GET["invno"] . "'";
