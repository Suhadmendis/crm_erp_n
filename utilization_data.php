...
if ($_GET["Command"]=="settle_inv"){
	...
	$ResponseXML .= "<uti_table><![CDATA[ <table  border=1  cellspacing=0>
        ...
        $sqlOuter = "select Inv_no, Incen_val from s_crnfrmtrn where crd_note = '" .$_GET["txtcrnno"]. "'"; 
        ...
        while ($rowOuter = mysql_fetch_array($resultOuter)){
                ...
                $sql="select * from s_salma where REF_NO='".$rowOuter["Inv_no"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0 $phrase ORDER BY SDATE";
		...
		$ResponseXML .= "<tr><td><div id=\"".$invno."\">".$row["REF_NO"]."</div></td>
						...
						<td><input type=\"text\"  class=\"text_purchase3_right\" name=".$settle." id=".$settle."  value=".number_format($rowOuter["Incen_val"], 2, ".", "")."  size=\"10\"/></td>
						<td><input type=\"text\"  class=\"text_purchase3_right\" name=".$remain." id=".$remain." value=".number_format(($row["GRAND_TOT"]-$row["TOTPAY"]-$rowOuter["Incen_val"]), 2, ".", "")." size=\"10\"/></td>
						</tr>";	
	...
	$additionalResponse = "";
	if ($isCashCredit == "yes"){
            $additionalResponse .= "<isCashCredit><![CDATA[yes]]></isCashCredit>";
        }else{
            $additionalResponse .= "<isCashCredit><![CDATA[no]]></isCashCredit>";
        }
        ...
        while ($rowOuter = mysql_fetch_array($resultOuter)){
                ...
                $sql="select * from s_salma where REF_NO='".$rowOuter["INVOICENO"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0 $phrase ORDER BY SDATE";
	...
	if ($i==1){
            ...
            while ($rowOuter = mysql_fetch_array($resultOuter)){
                    ...
                    $sql="select * from s_salma where REF_NO='".$rowOuter["C_INVNO"]."' and TOTPAY<GRAND_TOT and (GRAND_TOT-TOTPAY)>0 and CANCELL=0 $phrase ORDER BY SDATE";
	...
	$ResponseXML .= "<tr><td><div id=\"".$invno."\"></div></td>
				...
				$ResponseXML .= $additionalResponse;
				$ResponseXML .= " </salesdetails>";
				echo $ResponseXML;								
}
