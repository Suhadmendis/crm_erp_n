...
if ($_GET["Command"] == "new_inv") {
    ...
    $ResponseXML .= "<uniq><![CDATA[" . uniqid() . "]]></uniq>";
...
//         str_code, str_description, cur_qty, str_invno
        $sql = "Insert into tmp_stock_adjust_data (str_code, str_description, cur_qty, str_invno,uniq)values
			('" . $_GET['itemCode'] . "', '" . $_GET['itemDesc'] . "', " . $_GET['qty'] . ",'" . $_GET['tmpno'] . "','" . $_GET['uniq'] . "') ";
        $result = $conn->query($sql);
    }
    ...
    $i = 1;
    $mtot = 0;
    $sql = "Select * from tmp_stock_adjust_data where uniq='" . $_GET['uniq'] . "'";
...
//        exit ($sql);
        ...
        for ($x = 1; $x < $_GET['rowLength']; $x++) {
            $sql = "Insert into tmp_stock_adjust_data (str_code, str_description, cur_qty, str_invno, txt_jobno)values
			('" . $arrElement[$var1] . "', '" . $arrElement[$var1 + 1] . "','" . $arrElement[$var1 + 5] . "','" . $_GET["tmpno"] . "','" . $_GET["txt_jobno"] . "') ";
            $result = $conn->query($sql);
