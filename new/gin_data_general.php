...
function getno($type) {
    ...
    if ($type == "D") {
        $sql = "select GINMD from invpara";
        $result = $conn->query($sql);
        $row = $result->fetch();
        $tmpinvno = "000000" . $row["GINMD"];
        $lenth = strlen($tmpinvno);
        return $invno = trim("ISD/") . substr($tmpinvno, $lenth - 7);
    } else {
        ...
        $lenth = strlen($tmpinvno);
        return $invno = trim("ISN/") . substr($tmpinvno, $lenth - 7);
    }
}
    ...
    try {
        ...
        if ($row = $result->fetch()) {
            ...
            if ($_GET['type'] == "N") {
                ...
                $sql = "update invpara set GINMD=GINMD+1";
                $conn->exec($sql);
            }
        }
		...
		<tr class='success'>
            ...
            // $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowGlPost["l_code"] . "', " . $totCost . ", 'GISN', 'DEB', '$ayear', '" . $_SESSION['company'] . "')";
            // $conn->exec($sqlLedger);

            // $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowGlPost["l_code"] . "', " . $totCost . ", 'GISN', 'CRE', '$ayear', '" . $_SESSION['company'] . "')";
            // $conn->exec($sqlLedger);
        ...
        if ($NOD == "D") {
            $acc = "MINGeneralIssueDirect";
            $accCRE = "StockInHandAccoun";
        }else{
            $acc = "MINGeneralIssueAccountNorma";
            $accCRE = "StockInHandAccoun";
        }
        ...
        foreach ($conn->query($sqltempraw) as $rowtempraw) {
            ...
            if ($rowtempraw["cur_qty"]!=0) {
                        ...
                        $sqlraw = "select cost,".$acc." from s_rawmas where STK_NO='" . $rowtempraw["str_code"] . "'";
                        $rowraw = $conn->query($sqlraw)->fetch();
                        $costraw = $rowraw["cost"] * $rowtempraw["cur_qty"];

                        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='" . $rowraw[$acc] . "'";
                        $rowacc = $conn->query($sqlacc)->fetch();
                        $accname = $rowacc["C_NAME"];


                $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowraw[$acc] . "', " . $costraw . ", 'ISN', 'DEB', '$ayear', '" . $_SESSION['company'] . "', '" . $_GET["txt_jobno"] . "')";
                    $conn->exec($sqlLedger);
                ...
                if ($_GET["Command1"] == "getGl") {
                    ...
                    //DEB
                    $msg .= "<tr>";
                    $msg .= "<td style = 'width: 120px;text-align: left;'>" . $rowraw[$acc] . "</td>";
        ...
        foreach ($conn->query($sqltempraw) as $rowtempraw) {
            ...
            if ($rowtempraw["cur_qty"]!=0) {
                        ...
                        $sqlraw = "select cost,".$accCRE." from s_rawmas where STK_NO='" . $rowtempraw["str_code"] . "'";
                        $rowraw = $conn->query($sqlraw)->fetch();
                        $costraw = $rowraw["cost"] * $rowtempraw["cur_qty"];

                        $sqlacc = "select C_CODE,C_NAME from lcodes where C_CODE='" . $rowraw[$accCRE] . "'";
                        $rowacc = $conn->query($sqlacc)->fetch();
                        $accname = $rowacc["C_NAME"];
                ...
                $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowraw[$accCRE] . "', " . $costraw . ", 'ISN', 'CRE', '$ayear', '" . $_SESSION['company'] . "', '" . $_GET["txt_jobno"] . "')";
                    $conn->exec($sqlLedger);
                ...
                if ($_GET["Command1"] == "getGl") {
                    ...
                    //DEB
                    $msg .= "<tr>";
                    $msg .= "<td style = 'width: 120px;text-align: left;'>" . $rowraw[$accCRE] . "</td>";
                    $msg .= "<td style = 'width: 120px;text-align: left;'>" . $accname . "</td>";
                    $msg .= "<td style = 'width: 10px;text-align: left;'></td>";
                    $msg .= "<td style = 'width: 10px;text-align: left;'>" . number_format($costraw, 2, ".", ",") . "</td>";
                    $msg .= "</tr>";
