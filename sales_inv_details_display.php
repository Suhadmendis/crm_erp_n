...
<form name="form1" id="form1">            
  ...
  <tr>
    <td>&nbsp;</td>
    <td><input type="hidden" name="hiddencount" id="hiddencount" />
      <input type="text"  class="label_purchase" value="D/A No" disabled="disabled"/></td>
    <td><input type="text" disabled="disabled" name="dano" id="dano" value="<?php echo $row['dele_no']; ?>" class="text_purchase3" onkeypress="keyset('searchcust',event);" onfocus="got_focus('invno');"  onblur="lost_focus('invno');"  /></td>
    <td>&nbsp;</td>
	...
	echo "<table><tr>
        ...
        $mvatrate =  $row["GST"] ;
                                /////////
                                    ...
                                    $sql_data = mysql_query("Select * from view_sinvo_smas where REF_NO='" . $_GET["refno"] . "'") or die(mysql_error());
    while ($row1 = mysql_fetch_array($sql_data)) {

        if (($VAT_add == "1") or ($VAT_add == "2")) {

 
                $PRICE = $row1['PRICE'] / (1 + $mvatrate / 100);
             
        } else {
            $PRICE = $row1['PRICE'];
        }

        $subtot_wo_disc = (floatval($PRICE) * floatval($row1['QTY']));
        $disco = $subtot_wo_disc * floatval($row1['DIS_per']) / 100;
        $subtot = $subtot_wo_disc - $disco;

        $dis_per1 = $row1["Print_dis1"];
        $dis_per2 = $row1["Print_dis2"];
        $dis_per3 = $row1["Print_dis3"];

        echo "<tr>
                           	
                                                         <td>" . $row1['STK_NO'] . "</a></td>
  							 <td>" . $row1['BRAND_NAME'] . "? " . $row1['DESCRIPT'] ."</a></td>
							 <td>" . number_format($PRICE, 2, ".", ",") . "</a></td>
							 <td>" . $row1['QTY'] . "</a></td>
							 <td>" . $row1['DIS_per'] . "</td>
							 <td>" . number_format($subtot, 2, ".", ",") . "</a></td>";
