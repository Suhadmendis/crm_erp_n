        ...
        function nilsal() {
            ...
            while ($row_v = mysql_fetch_array($result_v)) {
				...
				if (($_SESSION['COMCODE']=="A") or ($_SESSION['COMCODE']=="B") or ($_SESSION['COMCODE']=="R")){
                	$strinv = "select sum(grand_tot/(1 +gst/100)) as gtot,brand,c_code,month(sdate) as month ,year(sdate) as year from s_salma where mid(REF_NO, 1, 2)='".$_SESSION['INVC']."' and  CANCELL='0' ";
				} else {
...
//                    } else {
        ...
        function othersales() {
			...
			if ($_SESSION['COMCODE']=="C"){
            	$strinv = "select sum(grand_tot) as gtot,brand,c_code,month(sdate) as month ,year(sdate) as year from s_salma where   CANCELL='0' ";
			} else {
				$strinv = "select sum(grand_tot) as gtot,brand,c_code,month(sdate) as month ,year(sdate) as year from s_salma where  mid(REF_NO, 1, 2)='".$_SESSION['INVC']."' and CANCELL='0' ";
			}	
            if ($_GET["cmbrep"] != "All") {
                $strinv .=" and sal_ex= '" . $_GET['cmbrep'] . "'";
            }
                ...
                if ($yy == "y") {
					...
					if ($_GET["chkdevelo_cus"]=="on"){
						if ($cat=="Z"){
                            ...
                            . "('" . $row_RSINVO01['c_code'] . "', '" . $row_rsVENDOR["NAME"] . "', '" . $lmt . "', '" . $cat . "', '" . ($mon1 - $Gmon1) . "', '" . ($mon2 - $Gmon2) . "', '" . ($mon3 - $Gmon3) . "', '" . $row_rsVENDOR["cus_type"] . "' , '" . $row_RSINVO01['brand'] . "', '".$_SESSION["CURRENT_USER"]."' )";
                    		$result_RSMONSALES = $db->RunQuery($sql_RSMONSALES);
						}
					} else {
						$sql_RSMONSALES = "insert into monsales(Cus_Code, cus_name, limit1, cat,  month1, month2, month3, C_TYPE,brand, user_id) values "
			...
			if ($_SESSION['COMCODE']=="C"){	
            	$strgrn = "select sum(amount) as gtot,CUSCODE,brand,month(sdate) as month ,year(sdate) as year from c_bal where  trn_type<>'INC'and trn_type<>'REC' and trn_type<>'PAY' and trn_type<>'ARN' ";
			} else {
				$strgrn = "select sum(amount) as gtot,CUSCODE,brand,month(sdate) as month ,year(sdate) as year from c_bal where  trn_type<>'INC'and trn_type<>'REC' and trn_type<>'PAY' and trn_type<>'ARN'  and flag1 <> '1' (mid(REF_NO, 5, 1)='".$_SESSION['COMCODE']."' or mid(REF_NO, 1, 1)='".$_SESSION['COMCODE']."')";
			}	
...
//                }
		...
		<th>3 Month Total</th>
           ...
           if ($_GET["chkdevelo_cus"]!="on"){
			...
			if ($limit>0){
				echo "<td align=\"right\"><b>" . number_format($month1_tot / $limit * 100, 2, ".", ",") . "</b></td>
