...
function setRate(id,stk,val) {
     xmlHttp = GetXmlHttpObject();
    if (xmlHttp == null) {
        alert("Browser does not support HTTP Request");
        return;
    }
var rateid = val.getAttribute('id');
totCal(rateid);
    var url = "gin_data_general_1.php";
    url = url + "?Command=" + "up_row";
    url = url + "&id=" + id;
    url = url + "&stk=" + stk;
    url = url + "&value=" + document.getElementById(rateid).value;
    // console.log(document.getElementById(rateid).value);
...
function totCal(rateid) {
    var val = document.getElementById(rateid).value;


    var id = rateid.slice(0, -4);
    var qty = document.getElementById(id+"qty").value;

    document.getElementById(id+"tot").value = val*qty;
