	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								$sql = mysql_query("SELECT * from s_purmas_stores where  REFNO like  '$letters%' order by id desc") or die(mysql_error());
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_purmas_stores where SUP_NAME like  '$letters%' order by id desc") or die(mysql_error()) or die(mysql_error());
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_purmas_stores where SUP_NAME like  '$letters%' order by id desc") or die(mysql_error()) or die(mysql_error());
