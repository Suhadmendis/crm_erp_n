...
function calrepo()
{
	...
	//$rsVENDOR = "select * from  vendor where CODE = '" . trim($_GET["txt_cuscode"]) . "' ";
		//$result_rsVENDOR =$db->RunQuery($rsVENDOR);
		//while($row_rsVENDOR = mysql_fetch_array($result_rsVENDOR)){
	
	$sql_tmp = "select * from monsales ";

	$sql_rsVENDOR = "select C_CODE, NAME,incdays, sum(GRAND_TOT) as tot  from  view_salma_vendor_brand where year(SDATE) = '" . date("Y", strtotime($_GET["DTPicker1"])) . "' and month(SDATE) = '" . date("m", strtotime($_GET["DTPicker1"])) . "' and CANCELL = '0' and delinrate = '2.5' group by C_CODE,NAME,incdays order by C_CODE ";
	...
	while($row_rsVENDOR = mysql_fetch_array($result_rsVENDOR)){
		...
		if($row_rschper = mysql_fetch_array($result_rschper)){
        	if ($_GET["cmbrep"] == "All") {
             	...
             	$sql_rs1 = "select * from c_bal where month(SDATE)=" . Month(DTPicker1) . " and year(SDATE)=" . Year(DTPicker1) . "  and CUSCODE='" . trim($row_rsVENDOR["C_CODE"]) . "' and CANCELL = '0' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' ";
                	...
                	if ($row_rs["Out1"] < 50) { 
								...
								if ($row_rsbrn_mas["delinrate"]==2.5){
                      				...
                      				if (is_null($row_rssalma["Deli_date"])==false){
                         				if (is_null($row_rssalma["st_chdate"])==false){
                             				$mdate = $row_rssalma["st_chdate"] - $row_rssalma["Deli_date"];
                         				} else {
                             				$mdate = $row_rssalma["ST_DATE"] - $row_rssalma["Deli_date"];
                         				}
						...
						$result_rsper =$db->RunQuery($sql_rsper);
			 			if($row_rsper = mysql_fetch_array($result_rsper)){
							...
							$sql_rsincen = "select * from ins_payment where  I_month ='" . date("m", strtotime($_GET["DTPicker1"])) . "'  and I_year='" . date("Y", strtotime($_GET["DTPicker1"])) . "' and cusCode = '" . trim($row_rsVENDOR["C_CODE"]) . "'order by id ";
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month1, month2, month3, limit, battery, target) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$msal.", ".$mret.", ".$month3.", ".$mpay.", ".$mpay_b.", ".$target_a.")";
				...
				$sql_rs = "select    sum(GRAND_TOT) as totsal, sum(GRAND_TOT-TOTPAY) as  out from s_salma where Accname != 'NON STOCK' and month(SDATE)=" . date("m", strtotime($_GET["DTPicker1"])) . " and year(SDATE)=" . date("Y", strtotime($_GET["DTPicker1"])) . "  and C_CODE='" . trim($row_rsVENDOR["c_CODE"]) . "' and SAL_EX = '" . $rep . "' and CANCELL = '0' ";
                	...
                	if ($row_rs["Out1"] < 50) { 
						...
						$result_rsper =$db->RunQuery($sql_rsper);
			 			if($row_rsper = mysql_fetch_array($result_rsper)){
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month1, month2, month3, limit, target) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$msal.", ".$mret.", ".$month3.", ".$mpay.", ".$target_a.")";
							$result_dealer =$db->RunQuery($sql_dealer);
	...
	if ($_GET["cmbrep"] == "") {
		...
		while($row_tmp1 = mysql_fetch_array($result_tmp1)){
        	...
        	if (is_null($row_tmp1["month3"]) == false) { $Flexcus[$mRow][4] = $row_tmp1["month3"]; }
		...
		while($row_tmp1 = mysql_fetch_array($result_tmp1)){
        	...
        	if (is_null($row_tmp1["month3"]) == false) { $Flexcus[$mrow1][4] = $row_tmp1["month3"]; }
...
function calrepo2014(){
	if ($_GET["Check1"] == "true") {
		...
		while($row_rsVENDOR = mysql_fetch_array($result_rsVENDOR)){
			...
			$result_rssal =$db->RunQuery($rssal);
			while($row_rssal = mysql_fetch_array($result_rssal)){
			...
			if (is_null($row_rs["totsal"])==false) {
               ...
               if (($Out < 50) or ($Out_bat < 50) or ($out_tube < 50)) { 
					...
					$result_rssalma =$db->RunQuery($rssalma);
					while($row_rssalma = mysql_fetch_array($result_rssalma)){
                        ...
                        if ($days > 0) {
							...
							$result_rsbrn_mas =$db->RunQuery($rsbrn_mas);
							if($row_rsbrn_mas = mysql_fetch_array($result_rsbrn_mas)){
                    ...
                    if ($mdate > 0) {
						...
						$result_rsper1 =$db->RunQuery($sql_rsper1);
						$row_rsper1 = mysql_fetch_array($result_rsper1);
						
						$result_rsper1 =$db->RunQuery($sql_rsper1);
						$count_rsper1 = mysql_num_rows($result_rsper1);
						
                        $sql_rsper1_50 = "Select * from intper where incen_year = 20112 and traget < '" . ($mpay + $mpay_50 + $m_pay_ut) . "' order by traget desc ";
						$result_rsper1_50 =$db->RunQuery($sql_rsper1_50);
						$row_rsper1_50 = mysql_fetch_array($result_rsper1_50);
						
						$result_rsper1_50 =$db->RunQuery($sql_rsper1_50);
						$count_rsper1_50 = mysql_num_rows($result_rsper1_50);
                        ...
                        if (($count_rsper1 > 0) or ($count_rsper1_50 > 0)) {
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month2, month3, tyre_inc, limit, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$mret.", ".$tyre_inc.", ".$tyre_inc.", ".$limit.", ".$target_a.", ".$target_bat_a.", 'Tyre')";
							$result_dealer =$db->RunQuery($sql_dealer);
						...
						$result_rsper1_bat =$db->RunQuery($sql_rsper1_bat);
						if($row_rsper1_bat = mysql_fetch_array($result_rsper1_bat)){
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month1, month3, battery_inc, battery, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$mpay_bat.", ".$bat_inc.", ".$bat_inc.", ".$mpay_bat.", ".$target_a.", ".$target_bat_a.", 'Battery')";
							$result_dealer =$db->RunQuery($sql_dealer);
						...
						$result_rsper1_tube =$db->RunQuery($sql_rsper1_tube);
						if($row_rsper1_tube = mysql_fetch_array($result_rsper1_tube)){
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month1, month3, battery_inc, tube, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$mpay_tube.", ".$tube_inc.", ".$tube_inc.", ".$mpay_tube.", ".$target_a.", ".$target_bat_a.", 'Tube')";
							$result_dealer =$db->RunQuery($sql_dealer);
						 ...
						 $result_rsper1 =$db->RunQuery($sql_rsper1);
						 if($row_rsper1 = mysql_fetch_array($result_rsper1)){
							...
							$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month2, month3, tyre_inc, limit, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$mret.", ".$tyre_inc.", ".$tyre_inc.", ".$mpay.", ".$target_a.", ".$target_bat_a.", 'Tyre')";
							$result_dealer =$db->RunQuery($sql_dealer);
		...
		if ($_GET["cmbrep"] == "") {
			...
			$result_tmp2 =$db->RunQuery($sql_tmp2);
			while($row_tmp2= mysql_fetch_array($result_tmp2)){
    ...
    ////////// test() //////////////////////////////////////////////////////////
		...
		$sql_tmp = "select * from dealer_inc ";
  		...
  		$result_rssales =$db->RunQuery($sql_rssales);
  		while($row_rssales= mysql_fetch_array($result_rssales)){
			...
			$result_rs_per =$db->RunQuery($sql_rs_per);
    		$row_rs_per= mysql_fetch_array($result_rs_per);
	    	...
	    	if (($row_rssales["sale"] > $row_rs_per["traget"]) and ($Out == "Ok") or ($out_b == "Ok")) {
				...
				$result_rs1 =$db->RunQuery($sql_rs1);
	    		$row_rs1= mysql_fetch_array($result_rs1);
				...
				if (($mpay > 0) or ($mpay_50 > 0) or ($mpay_b > 0)) {
					...
					$result_rsper =$db->RunQuery($sql_rsper);
    		    	$row_rsper= mysql_fetch_array($result_rsper);
			
					$result_rsper =$db->RunQuery($sql_rsper);
	    	    	$count_rsper= mysql_num_rows($result_rsper);
			
	    	        $sql_rsper_50 = "Select * from intper where traget <= '" . ($mpay + $mpay_50) . "' and incen_year = '20112' order by traget desc";
					$result_rsper_50 =$db->RunQuery($sql_rsper_50);
			   	   	$row_rsper_50= mysql_fetch_array($result_rsper_50);
				
					$result_rsper_50 =$db->RunQuery($sql_rsper_50);
		    	    $count_rsper_50= mysql_num_rows($result_rsper_50);
		            ...
		            if (($count_rsper > 0) or ($count_rsper_50 > 0)) {
						...
						$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month2, month3, tyre_inc, limit, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$ret.", ".$tyre_inc.", ".$tyre_inc.", ".$limit.", ".$target_a.", ".$target_bat_a.", 'Tyre')";
						$result_dealer =$db->RunQuery($sql_dealer); 
            		...
            		$sql_rsper_b = "Select * from intper where incen_year = 20111 and traget < '" . $mpay_b . "' order by traget desc ";
	           	 	$result_rsper_b =$db->RunQuery($sql_rsper_b);
	    	    	if($row_rsper_b= mysql_fetch_array($result_rsper_b)){
						...
						$sql_dealer = "insert into dealer_inc(cus_code, cus_name, month1, month3, battery_inc, battery, target, target_bat, c_type) values ('".$row_rsVENDOR["CODE"]."', '".$row_rsVENDOR["NAME"]."', ".$mpay_b.", ".$bat_inc.", ".$bat_inc.", ".$mpay_b.", ".$target_a.", ".$target_bat_a.", 'Battery')";
						$result_dealer =$db->RunQuery($sql_dealer); 
