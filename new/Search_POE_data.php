...
if ($_GET["Command"] == "pass_quot") {
    ...
    if ($row = $sql->fetch()) {
        ...
        $ResponseXML .= "<id><![CDATA[" . $row['rec_no'] . "]]></id>";
...
if ($_GET["Command"] == "search_custom") {
    ...
    if (isset($_GET["mstatus"])){
        if ($_GET["mstatus"] == "cusno") {
            ...
            $sql = "select rec_no,des from item_mas where  rec_no like '%$letters%' ";
        ...
        $sql = "select rec_no,des from item_mas";
    }
    $sql .= " ORDER BY rec_no limit 50";
    ...
    foreach ($conn->query($sql) as $row) {
        ...
        $ResponseXML .= "<tr>               
                              <td onclick=\"custno('$cuscode', '$stname');\">" . $row['rec_no'] . "</a></td>
