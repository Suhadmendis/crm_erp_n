...
if ($_GET["Command"] == "new_rec") {
    ...
    if ($_SESSION["dev"] != "") {
        ...
        $sql = "select ar_adv from invpara ";
        $result = $db->RunQuery($sql);
        $row = mysql_fetch_array($result);

        $tmprecno = "000000" . $row["ar_adv"];
        $lenth = strlen($tmprecno);

        $recno = trim("AD/19/") . substr($tmprecno, $lenth - 5);
        $_SESSION["recno"] = $recno;

        $sql = "Select ar_adv_t from tmpinvpara";
        $result = $db->RunQuery($sql);
        $row = mysql_fetch_array($result);
        $_SESSION["tmp_no_cashrec"] = $row["ar_adv_t"];

        $sql = "update tmpinvpara set ar_adv_t=ar_adv_t+1";
        $result = $db->RunQuery($sql);
        //  }

        /*  if ($_SESSION['dev'] == "1") {

          $sql="select ar_adv_t_M from invpara";
          $result =$db->RunQuery($sql);
          $row = mysql_fetch_array($result);

          $tmprecno="000000".$row["ar_adv_t_M"];
          $lenth=strlen($tmprecno);
          $recno=trim("CRN/ ").substr($tmprecno, $lenth-5);
          $_SESSION["recno"]=$recno;

          $sql="Select ar_adv_t_M from tmpinvpara";
          $result =$db->RunQuery($sql);
          $row = mysql_fetch_array($result);
          $_SESSION["tmp_no_cashrec"]="CRN/".$row["ar_adv_t_M"];

          $sql="update tmpinvpara set ar_adv_t_M=ar_adv_t_M+1";
          $result =$db->RunQuery($sql);
        ...
        /*  $sql="delete from tmp_inv_data where tmp_no='".$_SESSION["tmp_no_cashrec"]."'";
          ...
          $sql="select ar_adv_t,ar_adv_t_M from invpara ";
          $result =$db->RunQuery($sql);
          $row = mysql_fetch_array($result);

          if ($_SESSION['dev'] == "0") {
          $tmprecno="000000".$row["ar_adv_t"];
          $lenth=strlen($tmprecno);
          ...
          $result =$db->RunQuery($sql);
          $row = mysql_fetch_array($result);
          $_SESSION["tmp_no_cashrec"]="REO/ ".$row["ar_adv_t"];
          ...
          $result =$db->RunQuery($sql);
          $row = mysql_fetch_array($result);
          $_SESSION["tmp_no_cashrec"]="REM/".$row["ar_adv_t_M"];
...
//=============================================================================
        if ($recno != '') {
            ...
            $sql = "insert into s_crec(CA_REFNO, CA_DATE, CA_CODE, CA_CASSH, CA_AMOUNT, overpay, FLAG, pay_type, CA_SALESEX, CANCELL, tmp_no, DEPARTMENT, cus_ref, AC_REFNO, TTDATE, DEV, company, remark) values
      ('" . $recno . "', '" . $_POST["invdate"] . "', '" . $_POST["cuscode"] . "', " . $cashtot . ", " . $txtpaytot . ", " . $txtoverpay . ", 'ADV', '" . $_POST["paytype"] . "', '" . $_POST["salesrep"] . "', '0', '" . $_SESSION["tmp_no_cashrec"] . "', 'O', '0', '" . $_POST["ca_refno"] . "', '" . $_POST["dt"] . "',  '" . $_SESSION['dev'] . "' , '" . $_SESSION['company'] . "', '" . $_POST['cus_remark'] . "')";
            //echo $sql;
            $result = mysql_query($sql, $dbinv);
            if ($result != 1) {
                $sql_status = 1;
...
*/
      ...
      ('" . $recno . "', '" . $_POST["invdate"] . "', '" . $_POST["cuscode"] . "', '" . $row1["NAME"] . "', '" . $row["chqno"] . "', '" . $row["chqdate"] . "', '" . $row["chqbank"] . "',  " . $chqamt . ", '" . $_POST["salesrep"] . "', 'ADV', 'N', '" . $_POST["chqcollect"] . "', 0, '1', '0', 'O')";
                $result2 = mysql_query($sql2, $dbinv);
                if ($result2 != 1) {
                    $sql_status = 9;
                }
      ...
      ('" . $recno . "', '" . $_POST["cuscode"] . "', '" . $_POST["invdate"] . "', 'ADV', " . $cash_chq_tot . ")";
            $result = mysql_query($sql, $dbinv);
            if ($result != 1) {
                $sql_status = 10;
            }
      ...
      ('" . $recno . "', '" . $_POST["invdate"] . "', 'ADV', '" . $_POST["cuscode"] . "',  " . $_POST["txtoverpay"] . ", " . $_POST["txtoverpay"] . ", '" . $_POST["salesrep"] . "', '" . $_SESSION["dev"] . "', '".$mvatrate."', '".$_SESSION['company']."')";
            ...
            $sql2 = "update invpara set ar_adv=ar_adv + 1 ";
            $result2 = mysql_query($sql2, $dbinv);
            if ($result2 != 1) {
                $sql_status = 13;
            }
            ...
            if ($_POST["paytype"] == "Cash TT") {
                $ldate = $_POST["dt"];
                $ayear = ac_year($ldate);
                $sql20 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear, c_remarks) values ('" . trim($recno) . "', '" . $ldate . "', '" . trim($_POST["accno"]) . "', " . $fulltot . ", 'ADV', 'DEB', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear', '" . $_POST["cuscode"] . "')";
                //echo $sql;
                ...
                if($chqtot>0){
                    $sql201 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear, c_remarks) values ('" . trim($recno) . "', '" . $ldate . "', '12601002-000', " . $chqtot . ", 'ADV', 'DEB', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear', '" . $_POST["cuscode"] . "')";
                    //echo $sql;
                ...
                if($cashtot>0){
                    $sql202 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear, c_remarks) values ('" . trim($recno) . "', '" . $ldate . "', '12601003-000', " . $cashtot . ", 'ADV', 'DEB', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear', '" . $_POST["cuscode"] . "')";
                    //echo $sql;
              ...
              $sql_debt="select DebAdvAcc from vendor where code = '" . $_POST["cuscode"] . "'";
                $result_debt=mysql_query($sql_debt, $dbinv);
                $row_debt = mysql_fetch_array($result_debt);

                $c_code = $row_debt["DebAdvAcc"];         



            $sql19 = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, acyear, c_remarks) values ('" . trim($recno) . "', '" . $ldate . "', '" . $c_code . "', " . $fulltot . ", 'ADV', 'CRE', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', '$ayear', '" . $_POST["cuscode"] . "')";
            //echo $sql;
            ...
            $sql = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, l_year) values ('" . trim($recno) . "', '" . $_POST["dt"] . "', '" . trim($_POST["accno2"]) . "', " . $fulltot . ", 'ADV', 'CRE', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', " . date("Y") . ")";
            //echo $sql;
            $result = mysql_query($sql, $dbacc);
            if ($result != 1) {
                    $sql_status = 19;
            ...
            $sql = "insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, l_year) values ('" . trim($recno) . "', '" . $_POST["dt"] . "', '" . trim($_POST["accno"]) . "', " . $fulltot . ", 'ADV', 'DEB', '" . $l_lmem . "', '0', 'R', 0, '" . $_SESSION['company'] . "', '', '0', " . date("Y") . ")";
            //echo $sql;
            $result = mysql_query($sql, $dbacc);
            if ($result != 1) {
                    $sql_status = 20;
