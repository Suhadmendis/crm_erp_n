	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								$sql = mysql_query("SELECT * from s_salma where C_CODE='".$_SESSION["custno"]."' and CANCELL='0' and REF_NO like  '$letters%' limit 50") or die(mysql_error());
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_salma where C_CODE='".$_SESSION["custno"]."' and CANCELL='0' and CUS_NAME like  '$letters%' limit 50") or die(mysql_error()) or die(mysql_error());
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_salma where C_CODE='".$_SESSION["custno"]."' and CANCELL='0' and CUS_NAME like  '$letters%' limit 50") or die(mysql_error()) or die(mysql_error());
