...
if ($_GET["Command"] == "search_inv") {
    ...
    if ($_GET["mstatus"] == "invno") {
		...
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")&&($_GET["stname"]!="req")){
                        $sql = mysql_query("SELECT * from s_ordmas_1_1 where REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
			...
			$sql = mysql_query("SELECT * from s_ordmas_1_1 where cancel='0' and REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
		}	
    } else if ($_GET["mstatus"] == "customername") {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")&&($_GET["stname"]!="req")){
        	$sql = mysql_query("SELECT * from s_ordmas_1_1 where SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		} else if(($_GET["stname"]=="arn_1")){
			...
			$sql = mysql_query("SELECT * from s_ordmas_1_1 where cancel='0' and SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		}	
    } else {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
