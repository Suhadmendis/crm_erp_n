	...
	if($_GET["Command"]=="add_tmp")
		{
			...
			$ResponseXML .= "<msg><![CDATA[".$msg."]]></msg>";
				$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
                              <td width=\"90\"  background=\"\" ><font color=\"#FFFFFF\">Code</font></td>
                              <td width=\"380\"  background=\"\"><font color=\"#FFFFFF\">Description</font></td>
                               <td width=\"100\"  background=\"\"><font color=\"#FFFFFF\">UOM</font></td>
	...
	if($_GET["Command"]=="purord")
		{
			...
			if($row = mysqli_fetch_array($result)){
				...
				$ResponseXML .= "<JOB_CODE><![CDATA[".$row['JOB_CODE']."]]></JOB_CODE>";
			...
			//$ResponseXML .= $sql;
				...
				$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
                              ...
                              <td width=\"100\"  background=\"\">UOM</td>
			...
			while($row = mysqli_fetch_array($result)){				
             	$ResponseXML .= "<tr>                              
							 ...
							 <td >".number_format($row['qty'], 0, ".", ",")."</a></td>
                                                         <td id='incost" . $i . "' contenteditable=\"true\"></td>    
							 <td ><img src=\"images/delete_01.png\" width=\"20\" height=\"20\" id=".$row['str_code']."  name=".$row['str_code']." onClick=\"del_item('".$row['str_code']."');\"></td></tr>";
...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			$sql1="insert into s_ordmas_1(REFNO, SDATE, SUP_CODE, SUP_NAME, REMARK, DEP_CODE, JOB_CODE, DEP_NAME, REP_CODE, REP_NAME, cancel, S_date, LC_No, pi_no, Brand, tmp_no, vat_type) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET["supplier_code"]."','".$_GET["supplier_name"]."','".$_GET["remarks"]."','".$_GET["department"]."','".$_GET["Job_No"]."','".$dep_name."','".$_GET["salesrep"]."','".$rep_name."', '0','".$_GET["dte_shedule"]."','".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["brand"]."',  '".$_SESSION['tmp_no_purord']."', '$vat')";
			//echo $sql1;
			$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			...
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			while($row = mysqli_fetch_array($result)){
    			$sql1="insert into s_ordtrn_1(REFNO, SDATE, STK_NO, DESCRIPT, ORD_QTY, partno, CANCEL, tmp_no, RATE) values ('".$row["str_invno"]."', '".$_GET["invdate"]."', '".$row["str_code"]."','".$row["str_description"]."','".$row["qty"]."','".$row["partno"]."','0', '".$_SESSION['tmp_no_purord']."','".$row["rate"]."')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
  			}
		//$_SESSION["print"]=1;			
