...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	///////////////////////////////////// Registration /////////////////////////////////////////////////////////////////////////
		...
		if($_GET["Command"]=="new_inv")
		{
			...
			$sql="Select ORD_NO_1_1 from invpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$tmpinvno="000000".$row["ORD_NO_1_1"];
			$lenth=strlen($tmpinvno);
			$invno=trim("DODN/ ").substr($tmpinvno, $lenth-8);
			$_SESSION["invno"]=$invno;
			
			$sql="Select ORD_NO_1 from tmpinvpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$_SESSION["tmp_no_purord"]="DODN/".$row["ORD_NO_1"];
	...
	if($_GET["Command"]=="purord")
		{
			...
			$sql="select * from s_ordmas_1_1 where REFNO='".$_GET['invno']."' ";
			//$ResponseXML .= $sql;
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			...
			$sql="select * from s_ordtrn_1_1 where REFNO='".$_GET['invno']."' ";
			//$ResponseXML .= $sql;
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			while($row = mysqli_fetch_array($result)){
	...
	if($_GET["Command"]=="setord")
	{
		...
		$salesord1=trim("DODN/ ").$_GET["salesrep"].trim(" / ").$need;
...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			//echo $sql;
			if($row = mysqli_fetch_array($result)){
				$sql1="delete from s_ordmas_1_1 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
				
				$sql1="delete from s_ordtrn_1_1 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			} else {
				$sql="Select ORD_NO_1_1 from invpara";
				$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
				...
				$tmpinvno="000000".$row["ORD_NO_1_1"];
				$lenth=strlen($tmpinvno);
				$invno=trim("DODN/ ").substr($tmpinvno, $lenth-7);
				$_SESSION["invno"]=$invno;
				
				$sql1="update invpara set ORD_NO_1_1=ORD_NO_1_1+1";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
			}
			...
			$sql1="insert into s_ordmas_1_1(REFNO, SDATE, SUP_CODE, SUP_NAME, REMARK, DEP_CODE, DEP_NAME, REP_CODE, REP_NAME, cancel, S_date, LC_No, pi_no, Brand, tmp_no, vat_type) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET["supplier_code"]."','".$_GET["supplier_name"]."','".$_GET["remarks"]."','".$_GET["department"]."','".$dep_name."','".$_GET["salesrep"]."','".$rep_name."', '0','".$_GET["dte_shedule"]."','".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["brand"]."',  '".$_SESSION['tmp_no_purord']."', '$vat')";
			//echo $sql1;
			$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			...
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			while($row = mysqli_fetch_array($result)){
    			$sql1="insert into s_ordtrn_1_1(REFNO, SDATE, STK_NO, DESCRIPT, ORD_QTY, partno, CANCEL, tmp_no, RATE) values ('".$row["str_invno"]."', '".$_GET["invdate"]."', '".$row["str_code"]."','".$row["str_description"]."','".$row["qty"]."','".$row["partno"]."','0', '".$_SESSION['tmp_no_purord']."','".$row["rate"]."')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
  			}
		//$_SESSION["print"]=1;			
...
{
	$sql1="update s_ordmas_1_1 set cancel='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	
	$sql1="update s_ordtrn_1_1 set CANCEL='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	echo "Canceled!";
}	
