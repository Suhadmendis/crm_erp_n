...
if ($_GET["Command"] == "search_inv") {
    ...
    if ($_GET["mstatus"] == "invno") {
		...
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")){
                        $sql = mysql_query("SELECT * from s_ordmas_3 where REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else if(($_GET["stname"]=="arn_1")){
                        $sql = mysql_query("SELECT * from s_purmas_tmp_3 where CANCEL='0' and REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else {
			$sql = mysql_query("SELECT * from s_ordmas_3 where cancel='0' and REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
		}	
    } else if ($_GET["mstatus"] == "customername") {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")){
        	$sql = mysql_query("SELECT * from s_ordmas_3 where SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		} else if(($_GET["stname"]=="arn_1")){
                        $sql = mysql_query("SELECT * from s_purmas_tmp_3 where CANCEL='0' and SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else {
			$sql = mysql_query("SELECT * from s_ordmas_3 where cancel='0' and SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		}	
    } else {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
