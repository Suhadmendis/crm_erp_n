...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	///////////////////////////////////// Registration /////////////////////////////////////////////////////////////////////////
		...
		if($_GET["Command"]=="new_inv")
		{
			...
			$sql="Select ORD_NO_2 from invpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$tmpinvno="000000".$row["ORD_NO_2"];
			$lenth=strlen($tmpinvno);
			$invno=trim("ORD2/ ").substr($tmpinvno, $lenth-8);
			$_SESSION["invno"]=$invno;
			
			$sql="Select ORD_NO from tmpinvpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$_SESSION["tmp_no_purord"]="ORD2/".$row["ORD_NO"];
	...
	if($_GET["Command"]=="setord")
	{
		...
		$salesord1=trim("ORD2/ ").$_GET["salesrep"].trim(" / ").$need;
...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			//echo $sql;
			if($row = mysqli_fetch_array($result)){
				$sql1="delete from s_ordmas_2 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
				
				$sql1="delete from s_ordtrn_2 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			} else {
				$sql="Select ORD_NO_2 from invpara";
				$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
				...
				$tmpinvno="000000".$row["ORD_NO_2"];
				$lenth=strlen($tmpinvno);
				$invno=trim("ORD2/ ").substr($tmpinvno, $lenth-7);
				$_SESSION["invno"]=$invno;
				
				$sql1="update invpara set ORD_NO_2=ORD_NO_2+1";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
			}
			...
			$sql1="insert into s_ordmas_2(REFNO, SDATE, SUP_CODE, SUP_NAME, REMARK, DEP_CODE, DEP_NAME, REP_CODE, REP_NAME, cancel, S_date, LC_No, pi_no, Brand, tmp_no, REQNO, vat_type, type1) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET["supplier_code"]."','".$_GET["supplier_name"]."','".$_GET["remarks"]."','".$_GET["department"]."','".$dep_name."','".$_GET["salesrep"]."','".$rep_name."', '0','".$_GET["dte_shedule"]."','".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["brand"]."',  '".$_SESSION['tmp_no_purord']."', '".$_GET["reqno"]."', '$vat', '".$_GET["type"]."')";
//			echo $sql1;
			$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
...
//                                    $afterVaT = $row["rate"];
//                                }
                                $sql1="insert into s_ordtrn_2(REFNO, SDATE, STK_NO, DESCRIPT, ORD_QTY, partno, CANCEL, tmp_no, RATE) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$row["str_code"]."','".$row["str_description"]."','".$row["qty"]."','".$row["partno"]."','0', '".$_SESSION['tmp_no_purord']."','".$row["rate"]."')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
  			}
                        
                        $sql1 = "update s_ordmas_2_1 set cancel='1' where REFNO='" . $_GET["reqno"] . "'";
                        $result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ;
                        $sql1 = "update s_ordtrn_2_1 set CANCEL='1' where REFNO='" . $_GET["reqno"] . "'";
                        $result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ;
...
{
	$sql1="update s_ordmas_2 set cancel='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	
	$sql1="update s_ordtrn_2 set CANCEL='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	echo "Canceled!";
}	
