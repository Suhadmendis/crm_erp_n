...
<table width="800" border="1" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="600" align="center"><strong>Description</strong></td>
    <td width="100" align="center"><strong>Recieved</strong></td>
    <td width="100" align="center"><strong>Issued</strong></td>
  </tr>
  ...
  <?php
		...
		$rct = "select * from cash_book_summery where company='".$_SESSION['COMCODE']."' and trn_date <'" . $_GET["dtfrom"]  . "' order by trn_date desc";
		...
		if ($_SESSION['COMCODE']=="A"){
			...
			$rct = "select * from view_crec_sttr_invo_smas where CA_DATE='" . $_GET["dtfrom"] . "' and left(CA_REFNO, 1)='R' and pay_type='Cash' and  CANCELL='0' and CA_CASSH>0 ";
		} else if ($_SESSION['COMCODE']=="C"){
	...
	$rct = "select * from s_crec where CA_DATE='" . $_GET["dtfrom"] . "'  and FLAG='RET' and pay_type!='Cash TT' and CANCELL!='1'";
	$result_rct=mysql_query($rct, $dbinv);
	while ($row_rct = mysql_fetch_array($result_rct)) {
	 ...
	 if ($row_rct["CA_CASSH"]>0){
		...
		$result_ven=mysql_query($ven, $dbinv);
		$row_ven = mysql_fetch_array($result_ven);
		
		
		$ST_INVONO="";
		
		$sql_inv="select * from ch_sttr where ST_REFNO='".$row_rct["CA_REFNO"]."'";
	 	$result_inv=mysql_query($sql_inv, $dbinv);
        while ($row_inv = mysql_fetch_array($result_inv)){
			
			$ret = "select * from s_cheq where CR_REFNO='" . $row_inv["ST_INVONO"] . "'  ";
			$result_ret=mysql_query($ret, $dbinv);
			$row_ret = mysql_fetch_array($result_ret);
			$ST_INVONO=$ST_INVONO." ".$row_ret["CR_CHNO"];
		}
		
		echo "<tr>";
		echo "<td>Return Cheque Settlement - ".$row_ven["NAME"]." - ".$ST_INVONO."</td>";
		echo "<td align=right>".number_format($row_rct["CA_CASSH"], 2, ".", ",")."</td>";
		echo "<td></td>";
		echo "</tr>";
		$ret_amt=$ret_amt+$row_rct["CA_CASSH"];
	...
	$tot_sale=$tot_sale+$ret_amt;
	
	$rct = "select * from ledger where l_date='" . $_GET["dtfrom"] . "' and l_flag1='DEB'  and l_code='12502'";
	if ($_SESSION['COMCODE']=="A"){
		$result_rct=mysql_query($rct, $dbacca);
		while ($row_rct = mysql_fetch_array($result_rct)) {
			echo "<tr>";
			...
			$tot_sale=$tot_sale+$row_rct["l_amount"];
		}	
	} else if ($_SESSION['COMCODE']=="B"){
		$result_rct=mysql_query($rct, $dbaccb);
		while ($row_rct = mysql_fetch_array($result_rct)) {
			echo "<tr>";
			echo "<td>".$row_rct["l_lmem"]."</td>";
			echo "<td align=right>".number_format($row_rct["l_amount"], 2, ".", ",")."</td>";
		...
		$result_rct=mysql_query($rct, $dbaccd);
		while ($row_rct = mysql_fetch_array($result_rct)) {
			echo "<tr>";
			echo "<td>".$row_rct["l_lmem"]."</td>";
			echo "<td align=right>".number_format($row_rct["l_amount"], 2, ".", ",")."</td>";
		...
		$result_rct=mysql_query($rct, $dbaccc);
		while ($row_rct = mysql_fetch_array($result_rct)) {
			echo "<tr>";
			echo "<td>".$row_rct["l_lmem"]."</td>";
			echo "<td align=right>".number_format($row_rct["l_amount"], 2, ".", ",")."</td>";
	...
	$rct = "select * from ledger where l_date='" . $_GET["dtfrom"] . "' and l_flag1='CRE'  and l_code='12502'";
	if ($_SESSION['COMCODE']=="A"){
		$result_rct=mysql_query($rct, $dbacca);
		while ($row_rct = mysql_fetch_array($result_rct)) {
			echo "<tr>";
