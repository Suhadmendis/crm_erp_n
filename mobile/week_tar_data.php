...
include_once("connection.php");
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			if($row = mysql_fetch_array($sql)){		
				...
				$sql_date=mysql_query("Select min(Tardate) as min_Tardate, max(Tardate) as max_Tardate from week_tragets where refno='".$_GET['refno']."'") or die(mysql_error());
			...
			$sql_ven=mysql_query("delete from tmp_weekly_tar where ref_no='".$row['refno']."' ")or die(mysql_error());
				
			$sql=mysql_query("Select * from week_tragets where refno='".$_GET['refno']."'")or die(mysql_error());
			while($row = mysql_fetch_array($sql)){		
				
				$sql_ven=mysql_query("Select * from vendor where CODE='".$row['cus_code']."'")or die(mysql_error());
				$row_ven = mysql_fetch_array($sql_ven);	
				
				$sql_ven=mysql_query("Insert into tmp_weekly_tar (sal_ref, ref_no, tar_date, cus_code, cus_name, target, sale, remark)values 
			('".$row['rep']."', '".$row['refno']."', '".$row['Tardate']."', '".$row['cus_code']."', '".$row_ven['NAME']."', ".$row["Target"].", 0, '".$row['remark']."')")or die(mysql_error());
