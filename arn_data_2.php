...
if ($_GET["Command"] == "new_inv") {
    ...
    $sql = "Select ARN_TMP_2 from invpara where COMCODE='" . $_SESSION['company'] . "'";

    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    $tmpinvno = "000000" . $row["ARN_TMP_2"];
    $lenth = strlen($tmpinvno);
    $invno =  $_SESSION['company'] . trim("2GT") . substr($tmpinvno, $lenth - 8);
    $_SESSION["invno"] = $invno;

    $sql = "Select ARN from tmpinvpara";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    $_SESSION["tmp_no_arn"] = "2GA" . $row["ARN"];
...
if ($_GET["Command"] == "arn") {
    ...
    $sql = "select * from s_ordmas_2 where REFNO='" . $_GET['invno'] . "' ";
    //$ResponseXML .= $sql;
    $result = $db->RunQuery($sql);
    if ($row = mysql_fetch_array($result)) {
    ...
    //$ResponseXML .= $sql;
    //	$result =$db->RunQuery($sql);
    //	$sql="select * from s_ordtrn_2 where REFNO='".$_GET['invno']."' ";
    //$ResponseXML .= $sql;
    //	$result =$db->RunQuery($sql);
    //	while($row = mysql_fetch_array($result)){
    ...
    $mcou = 0;
    $sql = "Select count(*) as mcou from s_ordtrn_2 where REFNO='" . $_GET['invno'] . "'";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    $mcou = $row["mcou"] + 1;
...
//        $row_selling = mysql_fetch_array($result_selling);
		...
		$stkNo= trim($row['STK_NO']);
        $ResponseXML .= "<tr>                              
                             <td ><input type=\"text\" size=\"15\" name=" . $itemcode . " id=" . $itemcode . "   value=" . $stkNo . " class=\"text_purchase3\" disabled=\"disabled\"/></td>
							 ...
							 <td ><input type=\"text\" size=\"15\" name=" . $cost . " id=" . $cost . " value=" . $row['RATE'] . " class=\"text_purchase3\" onkeyup=\"cal_subtot('" . $i . "', '" . $mcou . "');\" disabled=\"disabled\"/></td>
...
if ($_POST["Command"] == "save_item") {
    ...
    if ($_POST["count"] > 0) {
        ...
        $sql = "select * from s_purmas_tmp_2 where REFNO='" . $_POST["invno"] . "'";
	  ...
	  ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST["invno"] . "', " . $_POST["total_value"] . ", " . $_POST["total_value"] . ", " . (-1 * $_POST["total_value"]) . ", '" . $_POST["supplier_code"] . "', '" . $supplier_name . "', '" . $_SESSION['dev'] . "', 0, 0, 0, '0', 'O', 'ARN', 'OVER')";
        ...
        */
            ...
            $sql1 = "insert into s_purmas_tmp_2(REFNO, SDATE, ORDNO, LCNO, pi_no, COUNTRY, SUP_CODE, SUP_NAME, REMARK, DEPARTMENT, AMOUNT, PUR_DATE,
...
//                }
//                $sql4 = "insert into s_purtrn_tmp_2(REFNO, SDATE, STK_NO, DESCRIPT, COST, MARGIN, SELLING, REC_QTY, FOB, DEPARTMENT, QTYINHAND, O_QTY, 
// Cost_c, acc_cost, acc_cost_c, brand, vatrate, DISCOUNT, DISCOUNT2, ret_qty, cost_seling, cost_margin, CANCEL, soldqty, days) values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST[$itemcode_name] . "', '" . $_POST[$itemname_name] . "', " . $cost . ", " . $margin_name . ", " . $_POST[$selling_name] . ", " . $_POST[$qty_name] . ", " . $fob_val . ", '" . $_POST["department"] . "', " . $QTYINHAND . ", " . $_POST[$ord_qty_name] . ", " . $COST_mas . ", '" . $acc_cost . "', '" . $acc_cost_c . "', '" . $_POST["brand"] . "', '" . $vatrate . "', ".$_POST[$discount_a_name].", ".$_POST[$discount_b_name].", 0, 0, 0, '0', '', 0)";
                $sql4 = "insert into s_purtrn_tmp_2(REFNO, SDATE, STK_NO, DESCRIPT, REC_QTY, COST) values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST[$itemcode_name] . "', '" . $_POST[$itemname_name] . "', " . $_POST[$qty_name] . ", $cost)";
			   	$result1=mysql_query($sql4, $dbinv);
				if ($result1==false){ $sql_status='41 ' . $sql4; }
                $sql1 = "update s_ordtrn_2 set REC_QTY=REC_QTY +'". $_POST[$qty_name] . "' where  STK_NO='" . $_POST[$itemcode_name] . "' and REFNO='" . $_POST["orderno1"] . "'";
//		echo $sql1;
                                $result1=mysql_query($sql1, $dbinv);
...
*/
//                $sql1 = "update s_ordmas_2 set cancel='2' where REFNO='" . $_POST["orderno1"] . "'";
//				$result1=mysql_query($sql1, $dbinv);
//				if ($result1==false){ $sql_status=9; }
//               
//
//                $sql1 = "update s_ordtrn_2 set CANCEL='2' where REFNO='" . $_POST["orderno1"] . "'";
//				$result1=mysql_query($sql1, $dbinv);
...
/*
        ...
        $sql = "select * from s_ordtrn_2 where REC_QTY != ORD_QTY and REFNO='" . $_POST["orderno1"] . "'";
        $result=mysql_query($sql, $dbinv);
        
        if(mysql_num_rows($result) == 0){
            $sql = "update s_ordmas_2 set cancel='1' where REFNO='" . $_POST["orderno1"] . "'";
            $result=mysql_query($sql, $dbinv);
            if ($result==false){ $sql_status=9; }
            
            $sql = "update s_ordtrn_2 set CANCEL='1' where REFNO='" . $_POST["orderno1"] . "'";
            $result=mysql_query($sql, $dbinv);
            if ($result==false){ $sql_status=9; }
        }
        
		if ($sql_status==0){
        	$sql1 = "update invpara set ARN_TMP_2=ARN_TMP_2+1 where COMCODE='" . $_SESSION['company'] . "'";
			$result1=mysql_query($sql1, $dbinv);
...
if ($_GET["Command"] == "pass_arnno") {
    $ResponseXML = "";
    $ResponseXML .= "<salesdetails>";
    $sql = "Select * from s_purmas_tmp_2 where REFNO='" . $_GET['arnno'] . "'";
    $result = $db->RunQuery($sql);
    if ($row = mysql_fetch_array($result)) {
        $ResponseXML .= "<REFNO><![CDATA[" . $row["REFNO"] . "]]></REFNO>";
        $ResponseXML .= "<SDATE><![CDATA[" . $row["SDATE"] . "]]></SDATE>";
        ...
        $mcou = 0;
        $sql = "Select count(*) as mcou from s_purtrn_tmp_2 where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        $row = mysql_fetch_array($result);
        $mcou = $row["mcou"] + 1;
        ...
        $i = 1;
        $tot = 0;
        $sql = "Select * from s_purtrn_tmp_2 where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        while ($row = mysql_fetch_array($result)) {
...
if ($_GET["Command"] == "cancel_inv") {
    ...
    $sql = "select CANCEL from s_purmas_tmp_2 where REFNO='".$_GET["invno"]."' order by id desc";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    //echo $sql;
    if ($row["CANCEL"]=="1"){
    ...
    $sql1 = "update s_purmas_tmp_2 set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_purtrn_tmp_2 set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_ordmas_2 set cancel='0' where REFNO='" . $_GET["orderno1"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_ordtrn_2 set CANCEL='0' where REFNO='" . $_GET["orderno1"] . "'";
    $result1 = $db->RunQuery($sql1);
    ...
    $sql1 = "select * from s_purtrn_tmp_2 where REFNO='" . $_GET["invno"] . "'";

    $result1 = $db->RunQuery($sql1);
    while ($row1 = mysql_fetch_array($result1)) {
        $sql2 = "update s_ordtrn_2 set REC_QTY=REC_QTY-" . $row1["REC_QTY"] . " where STK_NO='" . $row1["STK_NO"] . "' and REFNO = '".$_GET["orderno1"]."'";
